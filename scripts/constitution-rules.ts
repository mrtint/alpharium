 ...
 *
 * 원칙 IV(헌법 2.0.0)는 "모델 출력을 점수로 매기거나, 여러 모델을 비교하거나,
 * 품질을 자동 채점하는 평가/벤치마크 코드"를 제품에 들이는 것을 금지한다.
 *
 * 이 검사는 **설정 파일이나 소스 코드의 아키텍처 모듈 경계**만 본다. 모델을 부르지
 * 않고, 출력을 만들지 않고, 품질을 재지 않는다. 그래서 원칙 IV가 막으려는
 * "측정 장치"가 아니다.
 *
 * **여기에 넣지 않을 것**: 추론 속도 측정, 출력 품질 점수, 모델 간 비교, 프롬프트 평가.
 * 또한 다운로드 상태나 로딩 안내 등 **사용자 경험(UX) 개선을 위한 정상적인 UI/UX
 * 요소(진행률, 경과시간 표시 등)를 어휘 차단 정규식으로 통제하지 않는다**(045 G8 철회
 * 및 헌법 2.0.0 개정). 단어 철자가 아니라 "모델 평가/채점 장치인가"가 유일한 기준이다.
 * ─────────────────────────────────────────────────────────────────────────
 */
