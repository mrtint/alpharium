...
- **🔄 스펙 035에서 구현 (2026-09-07, 브랜치 `035-model-ready-welcome-naming`, 실기기 대기)**:
  ...
  - 기기 없는 테스트 **2562개 통과**(137 스위트), lint(eslint 0 error)·tsc·헌법 검사 위반 0·prettier 클린.
  - **⏳ 실기기 검증 미완**: 첫 실행 환영 연출(US1), 작명 네 곳 반영(US2), 설정 이름 변경·스냅샷(US3), 확인 실패 갈래, Maestro 신규 흐름(`.maestro/welcome-naming.yml`, `FLOWS` 등록 완료)·회귀. **건너뛴 실기기 테스트는 통과가 아니다**(원칙 V). 절차는 `specs/035-model-ready-welcome-naming/quickstart.md` §2.
