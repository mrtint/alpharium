...
- [ ] T005 **관측 여부 표현** 타입을 `src/signals/observation.ts`에 구현 — 「관측되지 않았음」이 「값이 없음」과 구별되는 형태. 값 자리를 비우는 것으로 대신하지 않는다 (001 FR-010, 003 FR-242) **선행 게이트: T006**
- [ ] T006 ⚖️ 원칙 IV **관측 여부** 스키마 검증 테스트를 `__tests__/contract/observation.test.ts`에 작성 — 미관측과 값-없음이 구별되고, 「걸음 수 0」과 「걸음 수 미관측」이 다른 상태임을 검사. 엔티티 전체의 형태는 T011a가 맡는다
- [ ] T007 [P] 성격 카탈로그(식별자·표시명·서술)를 `src/persona/catalog.ts`에 구현 (002 FR-140·FR-141)
- [ ] T008 [P] 퍼소나 엔티티를 `src/persona/persona.ts`에 구현 — 이름(1~20자)과 **성격 식별자**만 보유. 기기 식별자·관측 환경·이름 변경 이력을 포함하지 않는다 (001 FR-005·FR-006, 005 FR-404) **선행 게이트: T009**
- [ ] T008a 퍼소나 **최초 부여**를 `src/persona/assign.ts`에 구현 — 퍼소나가 없는 상태에서 이름과 성격 식별자를 앱이 부여한다. 사용자가 고르는 것이 아니다 (001 FR-002, 002 FR-140). 이름 **변경**은 US3 범위이므로 여기 넣지 않는다 **선행 게이트: T009**
- [ ] T008b 퍼소나 **저장·조회**를 `src/storage/persona-store.ts`에 구현 — 기기 식별자에 종속시키지 않고(001 FR-005, 005 FR-404), 동시에 **하나만** 활성으로 유지한다(001 FR-007). 관측 환경·이름 변경 이력을 담을 자리를 두지 않는다 (001 FR-006, 002 FR-142) **선행 게이트: T009**
- [ ] T009 ⚖️ 원칙 IV 퍼소나 지속성 테스트를 `__tests__/contract/persona-persistence.test.ts`에 작성 — 기기 식별자가 달라져도 이름·성격이 동일하고(001 SC-005), 저장된 것에 기기 식별자가 없으며(005 FR-404), 부여 후 조회 시 **활성 퍼소나가 정확히 하나**다(001 FR-007)
- [ ] T010 [P] 일별 집계 엔티티를 `src/signals/digest.ts`에 구현 — [data-model.md](data-model.md) 엔티티 2의 열 항목. **목록 밖의 항목을 추가하지 않는다** (003 FR-244) **선행 게이트: T011a**
- [ ] T011 [P] 일기 엔티티를 `src/inference/diary.ts`에 구현 — 날짜·퍼소나 귀속·본문 셋뿐. 제목·태그·기분·재료 요약을 두지 않는다 (004 FR-300·FR-301) **선행 게이트: T011a**
- [ ] T011a ⚖️ 원칙 IV **엔티티 스키마 검증** 테스트를 `__tests__/contract/entity-schema.test.ts`에 작성 — 집계는 [data-model.md](data-model.md) 엔티티 2의 **열 항목이 전부**이고 목록 밖 항목이 없음(003 FR-244), 일기는 **날짜·퍼소나 귀속·본문 셋이 전부**이며 제목·태그·기분·재료 요약이 없음(004 FR-300·FR-301)을 타입 수준에서 검사. 재료 요약은 저장 대상이 아니라 파생값임을 함께 확인한다 (004 FR-304)
- [ ] T012 **기록 묶음** 구조를 `src/storage/bundle.ts`에 구현 — 일기와 집계를 하나의 조작 단위로 다룬다. **원본 로그를 담을 자리가 존재하지 않아야 한다**(005 FR-402·FR-410). **가시성 상태를 구조에 포함한다** — 소프트 삭제 동작은 US5 범위 밖이나, T040·T041이 「보이는 기록」을 전제하고 001 FR-030이 숨겨진 기록과 보이는 기록의 같은 날짜 공존을 허용하므로 자리는 지금 있어야 한다 (001 FR-030·FR-034, [plan.md](plan.md) 「저장 계약은 US4·US5를 미리 만족시킨다」) **선행 게이트: T013**
- [ ] T013 ⚖️ 원칙 IV 기록 묶음 구조 테스트를 `__tests__/contract/bundle-structure.test.ts`에 작성 — 원본 로그를 담을 자리가 타입 수준에서 없음을 검사. 저장 시점 걸러내기로 대신하지 않았음을 확인 (005 FR-402). **가시성 상태가 구조에 존재하고 기본값이 「보임」임**을 함께 검사 (001 FR-030)
- [ ] T014 [P] `AIEngine` 인터페이스를 `src/inference/engine.ts`에 정의 — 모든 추론 접근이 이 경계를 통과한다. **인터페이스가 받는 것은 완성된 입력, 돌려주는 것은 원시 출력이다** — 프롬프트 구성·출력 해석·화자 판정이 구현체 안으로 들어갈 수 없는 형태여야 한다 (헌법 원칙 III 「어댑터의 책임은 호출뿐이다」 MUST)
- [ ] T015 **화자 판정기**를 `src/speaker/verify.ts`에 순수 함수로 구현 — [research.md](research.md) 결정 1의 어휘적 판정. **표지 목록은 비운 채 매개변수로 둔다**(T058에서 실측으로 채운다). `AIEngine` 구현체 안에 두지 않는다 (헌법 원칙 III, 004 FR-346~FR-348) **선행 게이트: T016**
...
- [ ] T018 [P] 저장소 형식 버전 식별을 `src/storage/version.ts`에 구현 (005 FR-403). **이행(마이그레이션) 절차 자체는 005의 몫이며 US1 범위 밖이다** — 여기서는 나중에 이행이 가능하도록 버전을 식별할 수 있게만 한다 (001 FR-036)
...
- [ ] T023 [US1] 권한 거부 분기를 `src/signals/permissions.ts`에 구현 — 크래시가 아닌 기능 축소. **모든 소스가 거부되어도 집계 한 덩어리를 산출한다**(비어 있음으로 판정) (003 FR-217, 헌법 원칙 V) **선행 게이트: T024**
- [ ] T024 [US1] ⚖️ 원칙 IV 권한 거부 테스트를 `__tests__/contract/permissions.test.ts`에 작성 — 소스별 거부와 전체 거부에서 집계가 산출되고 크래시하지 않음
- [ ] T025 [US1] 집계 산출기를 `src/signals/digest-builder.ts`에 구현 — **생성 요청 시점에** 만든다(상시 수집 금지). 항목별 상한과 시간대 세분도는 **매개변수로 두고 값을 비운다**(T060에서 실측) (003 FR-257·FR-260·FR-261) **선행 게이트: T028**
- [ ] T026 [US1] 규모 판정을 `src/signals/scale.ts`에 구현 — **여섯 항목**(걸음 수·활동 시간대·머문 장소·이동 여부·사진 목록·일정 목록)만 센다. 항상 존재하는 셋과 파생 항목(사진 총 개수)은 세지 않는다. 임계값은 **매개변수로 두고 값을 비운다**(T059에서 실측, 범위 2~6) (003 FR-271~FR-275) **선행 게이트: T027**
...
- [ ] T030 [US1] 프롬프트 구성을 `src/inference/prompt.ts`에 구현 — **미관측 항목은 아예 뺀다**(미관측이라고 알리지도 않는다). 회상 슬롯 자리를 두되 항상 비운다. 입력 구성 규칙과 추측의 강도는 **매개변수로 두고 값을 비운다**(T061에서 실측) (004 FR-318·FR-331, 001 FR-015) **선행 게이트: T031**
...
- [ ] T032 [US1] 클라우드 어댑터를 `src/inference/engines/cloud.ts`에 구현 — 온디바이스와 **동일한 스키마**를 쓰고 사진 원본을 전송하지 않는다 (헌법 원칙 I 개발 예외) **선행 게이트: T033**
- [ ] T033 [US1] ⚖️ 원칙 IV `AIEngine` 테스트를 `__tests__/contract/engine.test.ts`에 작성 — **정상·실패·형식 불량 응답** 셋 모두 (헌법 원칙 IV 명시 대상). **추론 축이 입력의 쓸모를 재판정하지 않음을 함께 검사** — 「적음」 판정이 붙은 집계와 「보통」 집계가 같은 경로로 처리된다 (001 FR-013b, 003 FR-282). **어댑터의 책임이 호출뿐임을 검사** — 어댑터에 완성된 입력을 주면 원시 출력을 그대로 돌려주고, 프롬프트를 구성하거나 출력을 해석하거나 화자를 판정하지 않는다 (헌법 원칙 III MUST)
...
- [ ] T035 [US1] 종료 사유 **다섯 가지** 구별을 `src/inference/failure.ts`에 구현 — 추론 실패 넷(입력 구성·모델 응답·형식·화자 위반, 004 FR-350)과 **사용자 취소/이탈**(001 FR-027). 취소는 추론 실패가 아니라 미완결이므로 004의 분류와 **구분해서** 다룬다. **입력 구성 실패 시 모델을 호출하지 않는다**(004 FR-352). 어느 경우든 부분 결과가 남지 않는다 **선행 게이트: T037**
...
- [ ] T037 [US1] 종료 사유 **다섯 가지** 테스트를 `__tests__/contract/inference-failure.test.ts`에 작성 — 다섯 경우 모두 **일기가 저장되지 않고**, 대체 텍스트가 그 자리를 채우지 않으며, 재시도가 가능함(001 SC-003, 004 FR-351·FR-353·FR-354). **취소·이탈에서는 부분 결과가 남지 않음**을 별도로 검사한다 (001 FR-027)

### 3c. 저장 축 (005)

- [ ] T038 [US1] 기록 묶음 저장을 `src/storage/repository.ts`에 구현 — **전부 성공하거나 전부 실패**한다. 일기만 있고 근거가 없는 상태를 만들지 않는다 (001 FR-032, 005 FR-411) **선행 게이트: T039**
- [ ] T039 [US1] ⚖️ 원칙 IV 저장 원자성 테스트를 `__tests__/contract/storage-atomicity.test.ts`에 작성 — 저장 도중 중단 시 일기와 집계가 함께 남거나 함께 남지 않음
- [ ] T040 [US1] 조회를 `src/storage/repository.ts`에 구현 — 보이는 기록의 목록, 특정 날짜의 기록 묶음 (005 FR-470) **선행 게이트: T041**
...
- [ ] T053 [US1] **일기 상세**를 `src/ui/screens/DiaryDetail.tsx`에 구현 — 본문·재료 요약·근거 집계·퍼소나 이름에 도달. **대조는 부가 기능이 아니라 필수 구성**이며 집계를 별도 영역으로 벗어나게 하지 않는다 (006 FR-504·FR-510, 001 FR-031) **선행: T054** (재료 요약 파생)
...
- [ ] T057 [US1] 통합 테스트를 `__tests__/integration/core-loop.test.tsx`에 작성 — [quickstart.md](quickstart.md) 시나리오 1(정상)·2(빈 집계)·3b(시각 무제약)·4(덮어쓰기와 재생성). 시나리오 4는 **두 번째 일기가 두 번째 집계와 짝지어져 있음**을 검사(001 SC-011), 시나리오 3b는 **생성이 시각을 이유로 거부되지 않음**을 검사 (001 SC-013)
