 ...
 *
 * ★ 037(헌법 1.6.0) — **지금은 로스터에 이 캐릭터뿐이다.** 견줄 대상이 있어서
 * 고른 것이 아니라, 넷이 자동 생성 부적합으로 나가고 남은 하나다(024·028·037).
 * 로스터가 늘면 이 값이 다시 **고르는** 자리가 된다 — 그때 근거는 위와 같이
 * 실측이어야 한다(헌법 원칙 III의 로스터 진입 기준).
 */
