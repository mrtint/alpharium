...
## 8. 배터리 최적화 예외(T014a) — Doze 화이트리스트로 대조

**Decision**: 정식 관측(예외 미적용) 라운드에서 15분 간격 등록에도
실제 실행 간격이 19시간 33분으로 벌어진 것(§7이 예측한 억제가 실측으로
확인됨)을 근거로, `adb shell dumpsys deviceidle whitelist
+com.anonymous.alpharium`로 이 앱을 Doze 화이트리스트(사용자가 설정
앱에서 "배터리 사용량 최적화 안 함"을 켜는 것과 동일한 시스템 상태)에
넣은 뒤, 화면을 끄고(`input keyevent KEYCODE_POWER`) 잠근 채로 같은
15분 등록이 실제로 15분 근방으로 도는지 대조 관측한다.

**Rationale**: FR-009가 요구하는 것이 정확히 이 대조다 — "배터리
최적화가 켜진 채(기본값)"와 "꺼진 채(예외)"를 같은 기기·같은 15분
등록으로 비교해야 억제의 원인이 Doze/앱 대기 버킷인지 다른 요인인지
가를 수 있다. 예외 적용 직후 `adb shell am get-standby-bucket`이
`5`(EXEMPTED, 가장 우호적인 버킷)로 즉시 바뀐 것과 `dumpsys
jobscheduler`의 해당 JobStatus가 `TIME=+10m7s899ms`(15분 요청과
근접한 값)로 갱신된 것이 이미 1차 신호다 — 정식 라운드에서 관측된
19.5시간 지연과 대비된다.

**Alternatives considered**: 실제 설정 앱 UI로 "배터리 사용량 최적화
안 함"을 사람이 켜기 — 최종 사용자가 실제로 밟을 경로와 더 가깝다는
장점이 있으나, adb의 `dumpsys deviceidle whitelist` 조작이 내부적으로
같은 화이트리스트 테이블을 갱신하므로 이 스파이크(기술 검증)에서는
동등하다고 보고 더 빠른 adb 경로를 택한다. 다음 스펙(020+)이 실제
기능화를 결정하면, 그때는 설정 화면으로 유도하는 온보딩 UX 자체가
과제가 된다(findings.md "다음 스펙에서 고려할 사항"에 남긴다).

## 9. 공식 문서와의 대조 — Context7로 확인한 Expo `expo-background-task` 문서

**Decision**: §7·§8의 실측 편차(기본값 19시간 33분, 예외 적용
10~32분)를 뒷받침하는 공식 근거를 Context7(`/websites/expo_dev_
versions_unversioned`, `expo-background-task` 문서)로 확인해 둔다.

**Rationale**: 문서가 이미 이 편차를 설계로 명시하고 있다 — "이
태스크는 즉시 실행이 보장되지 않으며, 시스템이 기기 상태에 따라 최적
시점에 스케줄한다"(When are background tasks run?), "`minimumInterval`은
최솟값으로 취급되며, 시스템이 배터리 사용과 깨우기를 최소화하도록
실행을 최적화한다... 기본값은 12시간, 허용 최솟값은 15분이다"
(BackgroundTaskOptions), "성공적인 실행에는 충분한 배터리 잔량과
활성 네트워크 연결이 필요하다"(실행 조건). 이 문서가 실측을 뒷받침하는
1차 근거이므로 별도의 조사 절로 남긴다 — findings.md의 "공식 문서
근거" 절이 이를 인용한다.

**Alternatives considered**: 없음 — 이미 확립된 실측 결과를 사후에
공식 문서로 검증하는 절이라 대안이 성립하지 않는다.
