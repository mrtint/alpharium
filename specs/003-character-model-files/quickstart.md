...
| F8 | **채록된 지문을 로스터에 옮겨 적는다** | 아래 참조 | FR-021, 원칙 V |

### F8 — 지문을 채록에서 검증으로 바꾸기

로스터의 `md5`는 비어 있고, 첫 내려받기는 **비교 대신 채록**을 한다
(contracts/readiness.md 「기준값이 아직 없을 때」). 그 값을 옮겨 적어야 다음부터 진짜
검증이 된다.

```bash
# 받은 뒤 기기에 남은 검증 결과에서 채록된 지문을 꺼낸다
adb shell "run-as com.anonymous.alpharium cat files/models/state.json"
```

`verdicts[].verifiedMd5`를 `src/models/roster.ts`의 해당 캐릭터 `md5`에 옮긴다. 옮기면
`roster.test.ts`의 "지문이 있다면 md5 모양이어야 한다"가 그 캐릭터에 실제로 걸리고,
이후 내려받기는 **채록이 아니라 검증**이 된다.

**옮기기 전에는 훼손을 잡지 못한다** — 비교할 대상이 없기 때문이다. 그래서 이 단계가
빠지면 SC-015(크기는 같고 내용이 다른 파일을 걸러낸다)가 아직 성립하지 않는다.
