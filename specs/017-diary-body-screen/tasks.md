...
- [X] T043 quickstart.md D2·D3·D3a·D4~D6를 실기기(dev 빌드)에서 수행한다:
  ...
  016의 T024와 같은 방식).
  [2026-08-24 1차 부분 확인 — 관측된 것: (1) 사진 있는 하루(08-22, 3장)를
  ...
  [2026-08-24 2차 — 남은 갈래를 마저 확인해 완료로 닫았다:
  (D2) 2026-08-22 일기(루이, 사진 3장)의 원본 첫 장(MediaStore
  `_id=44`, `2026-08-22-000.jpg`)을 `adb shell rm` + `content delete`로
  완전히 삭제한 뒤 같은 상세 화면을 다시 열어 사진 3장이 여전히 전부
  정상 렌더됨을 확인했다(리사이즈 사본이 원본과 독립적으로 보존됨,
  FR-002·P6).
  (D3a 로스터) 같은 하루(08-22, 08-21)를 루이(narrative)로도 생성해
  제목·본문을 읽었다 — quiet에 이어 **narrative에서도 마크다운 노출이
  재현됐다**(`**루이의 오늘 일기**`, `### 루이의 오늘 일기 - 2026-08-21`,
  research.md §9가 이미 지목한 대로)와 재조합 패턴("{캐릭터}의 오늘
  일기" 류)도 그대로 남아 있다 — `TITLE_INSTRUCTION` 개선이 quiet에서는
  마크다운을 없앴지만 narrative에서는 못 없앤 것으로, 캐릭터별 편차가
  실측으로 확인됐다(코드로 다시 거르지 않고 관측된 한계로 기록,
  원칙 IV). 본문에 "owner"(영문)가 섞여 나오는 것도 관측 — 011이 이미
  남긴 "캡션이 영어라 일기에 영어 단어가 섞인다"는 알려진 한계의
  재현이다.
  (D4 콜드 스타트) 루이로 재생성 시 글쓰기 56초가 관측됐다 — AGENTS.md의
  콜드 242초보다 훨씬 빠른데, 직전 시도(실수로 금동이가 선택됐던 시행
  착오)에서 이미 루이 모델이 한 번 로드됐을 가능성이 높아 **이번
  측정이 완전한 콜드스타트였다고 단정할 수 없다** — 정직하게 "웜에
  가까운 상태였을 가능성"으로 남긴다. 진행 중 화면("남겨진 기록들을
  모으는 중...", "찬찬히 눈에 담는 중...", "문장을 다듬어가며 적는
  중...")에는 이번에도 시간·퍼센트 정보가 전혀 없음을 스크린샷으로
  재확인했다(T9 재확인).
  (D5) `adb shell pm revoke`로 위치 권한을 거부한 상태에서 재생성 →
  "다닌 자리: 대표 장소 · 모른다 · 2곳"으로 `unknown` 귀결이 정확히
  화면에 반영됨을 확인했다(L5). 이어서 장소명 토글을 다시 켜자 시스템
  권한 다이얼로그가 재요청됐고, 이번에는 "허용 안함"(실제
  `permission_deny_and_dont_ask_again_button`, 즉 영구 거부)을 선택 —
  `dumpsys package`로 `USER_FIXED` 플래그가 붙는 것을 확인했다(진짜
  "다시 묻지 않음" 상태). 이 상태에서 토글을 껐다 다시 켜자 **권한
  다이얼로그가 다시 뜨지 않았고**, **토글은 사용자가 켠 값 그대로
  유지**됐다(L9 두 요구사항 모두 충족). 검증 후 `adb shell pm grant`로
  위치 권한을 원상 복구했다.
  (D6) 로스터 중 금동이·루이 두 캐릭터로 소요 시간 문장의 조사를
  확인 — 둘 다 받침 없는 이름이라 "는"이 자연스럽게 나온다(research.md
  §5가 이미 예견한 대로). 오드·샤오바이·모카 세 캐릭터는 모델이
  준비되지 않아(회색, 미다운로드) 이번 세션에서 실기기 확인이 불가능
  했다 — **로스터 5인 전수는 여전히 미완**이며, 다섯 이름 전부 받침이
  없다는 사실 자체는 `particle.test.ts`가 기기 없이 이미 커버한다
  (quickstart.md D6가 명시한 한계).
  종합: D2·D4(진행중 미노출)·D5는 완전히 확인됐고, D3a·D4(콜드스타트
  단정)·D6(로스터 전수)는 부분 확인으로 남는다 — 남은 부분은 위에
  정직하게 기록했다(원칙 V, "건너뛴 것은 통과가 아니다").]
...
- [X] T045 [P] 위반 주입으로 헌법 검사·계약을 재확인한다(개발 방식 관례):
  소요 시간 문장에 임시로 밀리초를 이어붙이거나 두 일기를 비교하는 코드를
  넣어 코드 리뷰/테스트가 잡는지 확인한 뒤 되돌린다(contracts/
  elapsed-time.md 「위반 주입」).
  [2026-08-24: 두 위반을 각각 주입했다. (1) 밀리초 이어붙이기
  ("...45초(45000ms)가 걸렸어요")를 넣었더니 **기존 테스트가 못 잡았다** —
  T10 테스트가 "45초" 부분 문자열만 정규식으로 확인해 뒤에 붙은 "(ms)"를
  못 봤다(실제 공백 발견). `diary-detail.test.tsx`에 `/\d+ms/` 부재를 직접
  확인하는 테스트를 새로 추가해 방어를 메웠다 — 재주입 후 새 테스트가 즉시
  실패로 잡는 것을 확인. (2) "지난번보다 빨라요/느려요" 비교 문구(모듈
  스코프 변수로 이전 실행의 writingMs를 기억하는 방식)를 넣었더니 **기존
  T8 테스트("비교 표현이 없다")가 정확히 잡았다** — 별도 방어 보강 불필요.
  둘 다 원래 코드로 되돌렸다(`git diff DiaryDetailScreen.tsx` 무변경 확인).
  `npm test`(1535개, 새 방어 테스트 포함)·`npm run lint` 전부 통과.]
