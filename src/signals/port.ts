...
export type PhotoFacts = {
   ...
   *
   * 사진 선별 시 "카메라 원본 vs 잡사진(스크린샷·저장·다운로드)" 분류의
   * 재료다. 그 이름이 잡사진 폴더인지는 판정하지 않는다 — 판정은
   * `src/vision/select.ts`의 순수 함수가 한다(경계).
   *
   * ─────────────────────────────────────────────────────────────────────────
   * **⚠️ `photosBetween()`은 이 필드를 채우지 않는다** — `filePathOf()`가
   * "`PhotoFacts`에 담지 않고 함수로 둔다. 함수로 두면 부르지 않는 쪽은
   * 경로를 얻을 수 없다"고 세운 011의 경계를 잇는다. 폴더 이름은
   * `folderNamesFor()`(아래)로 **필요한 자리에서만** 채운다 — 사진 선별을
   * 실제로 하는 경로(`on-device.ts`, 상한 초과인 하루)만 부른다. 004의 신호
   * 수집(장수만 세는)·사진 보기가 꺼진 요청·상한 이하인 하루(R1 빠른 경로)는
   * 폴더 이름이 필요 없어 `undefined`로 둔다.
   * ─────────────────────────────────────────────────────────────────────────
   *
   * **풀 수 없으면 `undefined`**: 경로를 못 얻음 · `content://` URI(폴더
   * 구간 없음) · 빈 문자열. `undefined`는 "분류 불가"라는 명시적 의미이며
   * (원칙 V의 none/unknown 계열), 선별에서는 "카메라 원본"과 같은
   * 취급(남긴다)을 받는다.
   */
  folderName?: string;
};
...
export interface PhotoPort {
  ...
  /**
   * 023 — 여러 사진의 상위 폴더 이름을 한 번에 얻는다.
   *
   * ─────────────────────────────────────────────────────────────────────────
   * **`filePathOf()`의 경계를 그대로 잇는다** — 폴더 이름을 `PhotoFacts`에
   * 미리 담지 않고 함수로 둔다. 부르지 않는 쪽(004 신호 수집, 사진 보기
   * 꺼짐, 상한 이하인 하루)은 폴더 이름을 얻지 않으며, 그래서 불필요한
   * `getUri()` 호출을 치르지 않는다.
   *
   * **배치인 까닭**: 상한 초과인 하루는 수십~수백 장이고, 장마다 따로 부르면
   * 왕복이 많다. 구현이 내부에서 병렬로 처리한다.
   * ─────────────────────────────────────────────────────────────────────────
   *
   * **어떤 경우에도 던지지 않는다** — `filePathOf()`와 같은 규칙. 한 장을
   * 못 읽으면 그 id는 맵에서 `undefined`이며("분류 불가"), 나머지는 채운다.
   *
   * @returns `photoId → 폴더 이름`. 풀 수 없는 id는 `undefined`이거나 맵에
   *   없다 — 부르는 쪽은 둘을 같게 다룬다.
   */
