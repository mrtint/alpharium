...
**Created**: 2026-08-18
**Updated**: 2026-08-18 — 「패키징」의 뜻이 확정되어 다시 씀. 영속이 끊긴 것을 발견해 Story 2를 더함. `/speckit-clarify`로 미결정 넷을 닫음
...
### 명확화 세션 (2026-08-18, `/speckit-clarify`)

네 가지를 물어 답을 받고 스펙에 반영했다. **셋은 스펙이 스스로 「정해야 한다」고 적어
둔 자리였다** — 미룬 결정이 남아 있었다는 뜻이므로 계획 전에 닫는 것이 맞았다.

1. **진단 화면의 `GenerationProbe`도 파이프라인을 거치도록 고친다.** 저장하지 않는
   생성 경로를 저장소에서 없앤다 — 남겨 두면 다음 사람이 본보기로 삼는다. FR-010을
   「모든 생성 경로」로 넓히고 FR-010a를 더했으며, SC-008b가 진단 경로까지 센다.
2. **저장이 실패해도 생성된 일기를 읽을 수 있게 한다.** 「저장하지 못했다」와 「앱을
   나가면 사라진다」를 함께 말한다(FR-012a·b, SC-008e). **FR-046의 「실패는 텍스트를
   반환하지 않는다」와 부딪히지 않도록 그것을 「*생성* 실패」로 좁혔다** — 저장 실패는
   생성이 성공한 뒤의 별개 사건이고 보여줄 글이 실제로 있다.
3. **환경 판정이 실패하면 앱은 뜨되 일기 기능을 막고 「이 빌드가 잘못 만들어졌다」로
   말한다**(FR-035a·b·c, SC-002a·b). `prod`로 간주하는 것은 001이 거부한 「기본값으로
   떨어지기」이며 원칙 V 위반이다. **사용자가 고칠 수 있는 문제가 아니므로 「다시
   시도하라」로 말하지 않는다.**
4. **읽을 수 없는 일기 파일은 목록에 「읽을 수 없다」로 남긴다**(FR-017a, SC-012a).
   조용히 빼면 「없었다」와 구분이 사라진다(원칙 V). SC-012의 예외 문구를 없애고
   별도 항목으로 세웠다.

**남은 미결정은 없다.** 스펙 전체에서 「정해야 한다」가 0건이다.
