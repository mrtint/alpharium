...
- [x] **자동 생성용 서술형·외국어 모델 재검토** (037 — 로스터를 검증된 하나(kanana/금동이)로 줄이고 헌법 v1.6.0 원칙 III에 「로스터 진입 기준」을 세웠다. 024·028의 실측 넷(exaone·qwen3·gemma3·hyperclovax)이 근거. 아래 14번 상세)
- [ ] **로스터 교체 후보 실측** (037 후속 — 로스터가 하나인 것은 완성이 아니라 현재 상태다(헌법 v1.6.0). 후보를 진입 기준(이 저장소의 프롬프트로 저장 가능한 일기를 실기기에서 안정적으로 낸다)에 걸어 통과한 것만 로스터에 넣는다. 037이 지목한 첫 확인 대상 — 중국어: 비추론 + 한국어 지시문에서 중국어 출력 + 1~2B(Qwen2.5-1.5B-Instruct), 영어: 한국어 지시문에서 영어 출력 + 1~3B(Llama-3.2-1B/3B-Instruct). 아직 재지 않았다)
...
- [x] **입력 프롬프트 텍스트 최적화** (036 — `my-ollama` 실험 리포트(kanana 1,242런 + Haiku·Sonnet·Opus 기준선 136편)의 결정 12를 저장소 소유자가 **E2SN 채택**으로 정했고, 그 화자를 `prompt.ts` 세 자리에 이식했다: 새 머리(`E2_RULES` 5줄·`E2_TITLE` 4문장·톤 줄은 금동이만), 문장형 신호 + 날짜 삭제(`sentenceSignalLines()`), 캡션 감싸기(`wrappedCaptionLines()`). **헌법 v1.5.0 선행 개정**(원칙 II 「화자는 본 것으로 주인의 하루를 짐작한다」). 판정 4갈래·`llama-port.ts` diff 0줄. my-ollama가 뽑은 30프롬프트와 **바이트 일치** 확인. **실기기 검증 완료**(2026-09-08, SM-S901N): 면책으로 안 끝남 5/6, 짐작 어미 없는 인물형 지어내기 0편, `writingMs` 19~22초(024 콜드 54초 대비 감소). 상세는 아래 18번. **루이·오드 실측(SC-004a)은 모델 파일 부재로 미수행이었고, 037이 그 둘을 로스터에서 빼며 무효가 됐다**)
- [x] **모델 준비 완료 연출 + 캐릭터 작명** (035 — 헌법 페르소나 조항 개정 동반, 실기기 검증 완료)
...
- [x] **초기 권한 획득 UI/UX 개선 + 작명·에셋 다운로드·첫 일기 병렬화** (040 — 로고 → 권한 자동 순차 → (작명 ∥ 다운로드) → liveness → 자동 첫 일기. `src/firstrun/` 경계 신설, 021·029·035 순수 로직은 재사용·무변경. 실기기 검증 완료 2026-09-11, 결함 둘 수정. 아래 24번 상세)
- [x] **모델 내려받기 OOM 해소** (041 — RN `fetch`에 응답 스트림이 없어 모든 구간이 `arrayBuffer()`로 힙에 올라가던 026의 결함. 구간 수신을 네이티브 `DownloadTask` + `Range`로 교체. 040 실기기 검증에서 발견, 같은 브랜치에서 수정. 10번 후속 — 상세는 10번 절)
- [x] **사진이 있는 하루는 VLM을 반드시 거친다** (042 — 「보지 않음」·「자세히 봄」 제거, 백그라운드·진단 경로까지 항상 캡션. 헌법 v1.7.0 선행 개정. **실기기 검증 D1~D7 + 완전 헤드리스까지 완료**, PR #68 머지)
- [~] **프롬프트로 화자를 바꾼다 — 강아지·고양이 페르소나** (`my-ollama` 후속 연구 `results/pet-speaker/`(커밋 `a39fae6`, kanana 120런) 이식. 모델은 kanana 그대로 두고 **프롬프트만으로 화자를 바꾼다** — 036이 이식한 E2SN의 뼈대·인식 구조·금지 조항을 유지하고 말투 층만 갈아 끼운다. **헌법 v1.8.0 선행 개정 완료**(2026-09-17): 페르소나가 프롬프트에서 말투·태도를 지시해도 되고(MAY), 한 모델 위에 캐릭터가 여럿 설 수 있다(MAY). 용어 「씨앗」→「모델의 성질」. **구현은 별도 스펙(043 예정)** — 진입 기준은 캐릭터마다 실기기 관측이라 my-ollama 120런은 후보를 좁힌 것이지 근거가 아니다. 아래 26번 상세)
...
- **✅ 후속 041 — 내려받기 OOM 해소 (2026-09-11, PR #65)**: 026이 남긴 결함을
  040 실기기 검증에서 발견해 같은 브랜치에서 고쳤다. **원인은 "어느 조건에서
  `res.body`가 null이 되는가"가 아니라 "언제나"였다** — RN 0.86의 전역 `fetch`는
  `whatwg-fetch` 폴리필이고(`Libraries/Core/setUpXHR.js:27` → `Libraries/Network/fetch.js:15`),
  그 `Body`에는 **`body` 속성 자체가 없다**(XHR 기반이라 `ReadableStream`이 없다).
  따라서 `expo-port.ts`의 `if (!res.body)` 폴백이 **늘 참**이 되어 모든 구간이
  `arrayBuffer()`로 약 380MB를 힙에 올렸고, 4구간이 동시에 그것을 해
  `OutOfMemoryError`가 났다(힙 한계 268MB, 1.39GB 지점). **그 아래 스트리밍 루프는
  한 번도 실행된 적이 없는 죽은 코드였다** — "구간 전체를 메모리에 담지 않는다"는
  주석이 사실이 아니었다. 011의 `has_media=0`, 013의 URI 계약 불일치, 020의 헤드리스
  `defineTask` 미등록과 같은 계열이고, **jest 대역은 `body`를 주므로 기기 없는
  테스트가 오히려 죽은 쪽을 검증했다.**
  **수정**: `expoRangeFetchPort.fetchRange` **하나만** 고쳤다 — 구간마다 임시 파일
  (`<key>.bin.seg<i>`)로 네이티브 `DownloadTask` + `Range` 헤더로 받고, 다 받으면
  `FileHandle.offset`을 옮겨 1MiB(`COPY_CHUNK_BYTES`)씩 최종 파일 제자리로 복사한다.
  네이티브가 디스크에 직접 쓰므로 **바이트가 JS 힙에 올라올 자리 자체가 없다** —
  "메모리에 담지 않는다"가 주석의 약속이 아니라 구조가 됐다. `RangeFetchPort` 계약·
  `src/models/segmented/`(순수 코어)·`port.ts`는 **한 줄도 안 고쳤고**, 026의 기존
  계약 테스트가 전부 그대로 통과하는 것이 그 증거다(SC-004). `remove()`·`bytesUsed()`가
  구간 임시 파일도 보게 했다(`leftoverNamesFor` — 수신 도중 죽으면 GB가 사용자 눈에
  안 보이는 채로 남는 것을 막는다, 008 계열). `tsc`가 `FileMode.Read`(없는 멤버, 실제는
  `ReadOnly`)를 잡았다.
  **실기기 검증 완료 (2026-09-11, SM-S901N, dev)**: a1을 지우고 처음부터 받아 1.5GB
  완주, `OutOfMemoryError` 0건(SC-001), md5 `d8506380…`로 지우기 전과 정확히 일치
  (SC-002), `.seg` 임시 파일 0개 잔존(FR-005). 기기 없는 테스트 153 스위트 2741 통과.
  **⏳ 미확인**: 1.5GB 수신 도중 멈췄다 이어받는 취소·재개 갈래는 유도하지 않았다
  (026·041의 계약 테스트가 잠금). 상세: `specs/041-model-download-oom/`.
...
- **✅ 039에서 확장 (2026-09-11, 사용자 요청)**: 038 실기기 검증 직후
  ...
  겪는 회귀다.
  - **실기기 검증 완료(2026-09-11, SM-S901N/Galaxy S22, dev)**.
    quickstart.md 2-1~2-5 육안 확인: 문구 글자 단위 노출, 완료 후 정지
    유지, 결과 화면 전환 시 잔상 없음, 화면 구성 무변경, 탭 무반응
    전부 확인.
  - **★ Maestro 조사에서 예측한 위험이 실측으로 확인됨**:
    `writing-monologue.yml`·`writing-monologue-expansion.yml`의
    `runFlow: when: visible: "쓰고 있다"`가 실기기에서 **실제로
    SKIPPED**됐다 — 독백 검증(FR-001·003·004·009·010·013·014)이 한
    건도 실행 안 된 채 흐름 자체는 실패로 안 잡히는 상태였다(원칙 V가
    경고하는 정확한 상황). `when` 조건을 타이핑과 무관하게 즉시
    렌더되는 "그만두기" 버튼으로 교체해 두 파일 모두 수정.
  - **수정 후에도 완전한 재현은 안 됨 — 037 로스터 축소가 낳은 구조적
    제약**: quiet(kanana)이 너무 빨라(콜드·웜 모두) `tapOn` 직후
    "그만두기"가 뜨는 순간조차 Maestro가 못 잡고 계속 SKIPPED됐다.
    037로 로스터가 kanana 하나뿐이라 narrative(루이) 같은 느린
    캐릭터로 대체 검증할 수 없다 — 이 파일 자체의 기존 주석("quiet처럼
    빠른 캐릭터는 독백이 스쳐 지나가 SKIPPED가 될 수 있다. 그것은
    실패가 아니다")이 예견한 상황이 037 이후 사실상 유일한 경로가
    됐다. 039의 회귀가 아니므로 추가 수정하지 않음.
  - Maestro 18흐름 재실행에서 나머지 실패 9개는 전부 039와 무관: 캐릭터
    이름이 사용자 지정("금동이"→"오구삼춘")된 이전 세션 잔여 상태,
    037의 알려진 로스터-하나 실패, `unified-permission-onboarding.yml`
    의 `pm clear`가 모델 파일까지 초기화한 상호오염(024·038 기록과
    동일 계열) — 검증 중 지워진 모델은 온보딩에서 재다운로드로 복구.
  - release 재확인 불필요(새 네이티브 모듈 0, 012).
  - 상세: `specs/039-writing-monologue-typewriter/`.

### 24. 초기 권한 획득 UI/UX 개선 — 로고 + 자동 권한 요청 + 작명·다운로드·첫 일기 병렬화 — ✅ 040에서 구현
...
- **✅ 스펙 040에서 구현 (2026-09-11, 브랜치 `040-onboarding-parallel-setup`, PR #65, 실기기 검증 완료)**:
  - **흐름이 재배치됐다**: 로고(`LOGO_DISPLAY_MS` 1.5초, 자동 전환) → 권한 목적
    설명 + 시스템 팝업 자동 순차 → (작명 ∥ 백그라운드 모델 다운로드) → liveness →
    **첫 일기 자동 생성**. 021의 단계별 [허용]/[건너뛰기] 두 버튼이 시스템 팝업
    하나의 결정으로 대체됐다(SC-001).
  - **새 경계 `src/firstrun/`** — 021·035와 같은 패턴(순수 판정 + `checkFirstRunFile`
    경계 검사). 040이 만든 것은 판정 셋(`resolveFirstRunStage`·`shouldShowLogo`·
    `shouldAutoGenerate`)과 조립 순서(`App.tsx` 게이트)뿐이며, **021의 `requirements.ts`·
    029의 `essential-assets.ts`·035의 `liveness.ts`·`naming.ts`는 한 줄도 안 고쳤다.**
  - **위 설계 긴장 8개의 답**: (1) 건너뛰기 = 시스템 팝업의 [거부], 배터리 예외는
    팝업으로 만들 수 없어 **전용 안내 화면 + 설정 진입 경로**로 흐름 안에 남겼다.
    (5) `completed`는 021처럼 전부 거부해도 `true`. (6) 다운로드는 화면 생명주기와
    분리해 돌리고, 작명이 먼저 끝나면 **그만두기 없는 대기 화면**에 머문다(필수
    에셋이라 007의 그만두기를 적용하지 않는다). (7) **liveness는 유지**하고 그
    다음에 자동 생성 — 실패 시 035의 기존 안내 갈래를 재사용한다(FR-008a).
    (8) 정오 게이트 이전이면 자동 생성을 **시도하지 않고** 기존 안내를 보이며,
    최초 실행에만 적용되므로 덮어쓰기 확인은 구조적으로 타지 않는다.
  - **자동 생성은 `schedule/task.ts`의 `runAutoDiaryTask`를 재사용하지 않는다** —
    그 함수는 `settings.enabled`와 목표 시각 창을 보는데, 040의 트리거는 "설정을
    막 끝낸" 1회성 이벤트라 그 설정과 무관하게 항상 시도해야 한다.
    `app/wiring.ts`의 `triggerFirstRunAutoDiary()`가 `pipeline.run()`을 직접 부른다.
  - **★ 실기기에서만 드러난 결함 둘** — 기기 없는 테스트 2700여 개가 통과한 채로
    둘 다 통과했다(011 `has_media=0`, 013 URI 계약 불일치, 020 헤드리스
    `defineTask` 미등록과 같은 계열). (1) **권한 결정 후 `completed`가 저장되지
    않았다** — 021은 마지막 [시작하기] 버튼이 플래그를 세웠는데 040은 그 버튼에
    도달하지 않는다. **화면을 떠나는 자리가 바뀌면 그 화면이 저장하던 것도 함께
    옮겨야 한다.** (2) **자동 생성이 끝나도 홈이 목록을 다시 읽지 않았다** —
    `DiaryHomeScreen`은 마운트·`AppState` 변화·자기가 돌린 생성에서만 `refresh()`를
    부르는데 040의 자동 트리거는 셋 중 어디에도 없다. 파일에는 일기가 있는데
    화면은 "아직 일기가 없다"였다.
  - **a1(1.5GB) 다운로드 OOM은 040의 결함이 아니었다** — `src/models/`는 040에서
    변경 0줄이고 마지막 변경은 026이다. **041에서 고쳤다**(10번 절 참조).
  - 실측 함정: 로고 1.5초는 스크린샷으로 놓치기 쉽다(상수를 임시로 6000으로
    올려 관찰). `clearState`는 앱 데이터만 지우고 **OS 권한은 그대로 둔다** —
    이미 부여된 기기에서는 그 스텝들이 자동으로 지나간다. `assertNotVisible`은
    `timeout`을 받지 않는다(Maestro 2.8.0 — 흐름 자체가 파싱되지 않는다).
  - 기기 없는 테스트 152 스위트 2734 통과, lint·헌법 검사·prettier 클린.
  - **⏳ 미확인으로 남은 것**: FR-009의 거부 판정 갈래(자동 생성이 두 번 다
    성공해 관측 못 함 — 계약 테스트로 갈음), liveness 실패 갈래(파일 크기로 먼저
    걸러져 유도 불가 — 035 계약 테스트 L3으로 갈음), Maestro `first-run-flow.yml`
    자동화(`clearState` 직후 기기 서버가 죽는다 — F1~F5는 손으로 전부 확인),
    021 `unified-permission-onboarding.yml`의 M5(모델을 미리 받아 둔 기기에서는
    그 assert가 성립하지 않는다 — 흐름 주석이 명시한 예외, 모델 없는 기기에서
    재확인 필요). 상세: `specs/040-onboarding-parallel-setup/`.

### 25. 사진이 있는 하루는 VLM을 반드시 거친다 — 「보지 않음」·「자세히 봄」 제거

- **구현 결과 (042, 2026-09-14)**: `VisionSetting`을 `"quick"` 하나로 좁히니 `tsc`가
  **72곳**을 짚었고 그것이 그대로 작업 목록이 됐다(037 방법). **닫은 경로는 셋이
  아니라 넷이었다** — 설정의 「보지 않음」, `auto`의 내려감, 그리고 조사 중 드러난
  **백그라운드 자동 생성**(`task.ts`가 「자동」·설정없음을 전부 `"none"`으로 떨궈
  **기본 설정 사용자의 자동 일기는 사진을 한 장도 안 봤다**)과 **개발자 탭 「지금
  생성」**(`GenerationProbe`의 기본값이 `"none"`). 뒤의 둘은 이 로드맵 항목의 고칠
  자리 목록에 없던 것이다. 함께 드러난 것 둘 더 — `VisionOutcome.skipped`가 제품
  코드에서 한 번도 반환되지 않는 **죽은 갈래**였고(6→5로 제거), `on-device.ts`의
  가드 순서가 틀려 시뮬레이터에서 「네이티브 모듈 없음」이 「사진 못 봄」에 가려지고
  있었다(「언제나 본다」가 되면서 비로소 보였다). **★ `tsc`는 유니온을 넓히는 위반을
  구조적으로 못 잡는다** — `"none"`을 되살려도 0 오류로 통과하므로 신규 계약 테스트
  `photo-vision-always.test.ts`가 유일한 방어이며, 위반 주입 7종 전부 잡히는 것을
  확인했다. 기기에 남은 `vision-setting.json`은 읽지도 지우지도 않는다(008·037 계열).
  기기 없는 테스트 152 스위트 2704개 통과, `tsc` 0, 헌법 검사 위반 0, prettier 클린.
  **실기기 검증 완료**(2026-09-14~15, SM-S901N / Android 16 / dev debug) — D1~D7과
  Maestro 세 흐름 전부 통과. **D5(백그라운드)가 이 기능의 실질이었고 확인했다**:
  사진 있는 하루를 **설정 조작 없이** 자동 생성이 집어 `has_media=1` 3회,
  `visionMs` 17996, 화면 표시 `결과: ran`. **잡 스케줄러가 스스로 깨우는 완전
  헤드리스**도 따로 확인했다(13:47 발화 → 캡션 3회 → 저장, `createdAt`이 `doWork`
  시각과 일치, 구간 내 `wm_on_resume_called`·`App is in the foreground` 0건).
  0장인 하루는 VLM을 안 열고(`has_media`·`tokenizeWithMedia`·`processMedia` 0회,
  모델 적재 1회), 권한을 회수하면 `none`이 아니라 `unknown`으로 「사진: 모른다」가
  나온다(원칙 V). **두 함정을 기록으로 남겼다** — 020의 시각 게이트
  (`isNearTarget`, 목표 12시의 창 `[12,15)`)가 창 밖 세션을 `skipped`로 막고,
  **`am force-stop`은 WorkManager 잡을 함께 취소**해 헤드리스 라운드를 통째로
  무효로 만든다(전경을 벗어나려면 홈 버튼). **헤드리스가 포그라운드보다 3.7배
  느리다**(`writingMs` 36.8초 → 137.5초) — `GENERATION_TIMEOUT_MS` 여유가 좁다.
  상세: `specs/042-photo-vision-always/`(`findings.md` §3).

- **배경** (2026-09-14 사용자 요청): 지금은 사진이 있는 하루여도 VLM을 안 거치는
  경로가 둘 있다. (1) 설정 탭 「사진 보기」에서 `none`(사진을 보지 않음)을 고르면
  영구히 꺼진다, (2) `auto`가 그 날 사진 신호가 없으면 `none`으로 내려간다
  ([resolve-generation.ts:88-95](../../src/app/resolve-generation.ts#L88-L95), 029 R5).
  이 항목은 **VLM을 반드시 사용하게 하는 조건만 남긴다** — 사진이 1장이라도 있으면
  VLM이 무조건 돌고, 사용자가 그것을 끌 수 없게 한다.

- **확정된 두 결정** (2026-09-14 사용자):
  1. **사진 0장인 하루는 엔진을 열지 않는다.** 011이 세운 「사진이 0장이거나 권한이
     없으면 VLM을 아예 열지 않는다(캡션 시도 0회)」를 유지한다 — 볼 것이 없는데 여는
     것은 적재 시간만 쓰고 얻는 신호가 0이다. 없애는 것은 **사용자 설정의 `none`과
     `auto`의 내려감 분기**이지 「볼 것이 없으면 안 연다」가 아니다.
  2. **`detailed`(자세히 봄)를 없애고 `quick` 하나로 고정한다.** 아래 「detailed를
     빼는 근거」 참조.

- **`detailed`를 빼는 근거** (2026-09-14 조사):
  - **모델 차이가 아니다.** `quick`·`detailed`는 같은 VLM(LFM2.5-VL 450M) 하나를 쓰고,
    바뀌는 값은 `initMultimodal`에 넘기는 `image_max_tokens` 하나뿐이다 —
    `quick: 256` / `detailed: 1024`([vision-port.ts:49-52](../../src/vision/vision-port.ts#L49-L52)).
    캡션 프롬프트·샘플링(`temperature` 0.1)·사진 상한(8장)·리사이즈(1024px)는 전부
    동일하고, 깊이 선택은 [on-device.ts:221](../../src/inference/on-device.ts#L221) 한 줄이다.
  - **두 값은 재진 적이 없다.** 소스 주석이 스스로 그렇게 적어 뒀다 — "⚠️ 두 값은
    짐작이다(원칙 V). 주석의 권장 범위 안이라는 것이 유일한 근거이며, 실제로 결과가
    다른지는 quickstart D6이 확인한다. 다르지 않으면 값을 벌린다." **그 D6이 수행된
    기록이 없다.**
  - **013 이후로는 효과 자체가 의심스럽다.** `image_max_tokens`는 장당 상한이 아니라
    **타일당** 상한이고, 캡션 시간을 정하는 것은 이 값이 아니라 해상도였다(013 실측).
    1024px 리사이즈가 들어간 뒤로는 장당 IMAGE 청크가 1개라 `detailed`의 1024가
    실제로 무엇을 더 사는지가 불확실하다.
  - **지어내기를 늘릴 수 있다는 미확인 관측이 붙어 있다.** AGENTS.md 011 절 —
    "캡션 깊이(`image_max_tokens`)가 클수록 더 지어낸다는 관측이 있다(검은 이미지
    기준)". 진짜 사진에서 같은지는 미확인. 원칙 II가 걸린 자리다.
  - 종합: **근거가 재진 적 없고, 효과가 의심스럽고, 원칙 II 위험이 붙은 다이얼**을
    사용자에게 노출하고 있다. 재서 살릴지는 나중에 정해도 되지만 지금 노출할 이유가 없다.

- **★ 헌법 개정이 선행한다** — Governance("원칙을 어기려면 헌법을 먼저 고친다. 예외를
  코드에 몰래 두지 않는다 MUST NOT")가 요구한다. 헌법 「사진과 시각 처리」 절이 지금
  이렇게 적혀 있다:

  > 사진 이해 방식은 캐릭터가 아니라 설정으로 둔다(MUST). 사용자가 시각 인코더를
  > 고르게 하지 않는다(MUST NOT). 사용자에게는 "사진을 보지 않음 / 빠르게 봄 /
  > 자세히 봄" 정도의 선택으로 제시한다.

  세 갈래 선택을 **명시적으로 예시하고 있으므로** 코드를 먼저 고치면 헌법이 뒤따라오는
  모양이 된다. 037(로스터를 하나로)이 헌법 v1.6.0을 코드보다 먼저 개정한 것과 같은
  자리다. 개정할 때 다룰 것:
  - "설정으로 둔다(MUST)"를 유지할지 — 선택지가 하나만 남으면 그것은 더 이상 설정이
    아니다. 「사진 이해 방식은 캐릭터가 아니다」(MUST NOT 쪽)는 살리고 「설정으로
    제시한다」를 조정하는 형태가 유력하다.
  - "사용자가 시각 인코더를 고르게 하지 않는다(MUST NOT)"는 **그대로 살린다** — 이
    변경은 그 방향을 오히려 강화한다.
  - "캐릭터와 시각 설정의 조합에 따라 결과가 달라진다. 모든 조합이 똑같이 좋다고
    가정하지 않는다(MUST NOT)" — 조합이 (로스터 1) × (깊이 1)로 줄어든다는 사실을
    반영할지.
  - 버전: MUST/MUST NOT 조항이 늘지 않고 확정 값이 바뀌는 것이므로 1.1.0·1.6.0 선례
    (「조항이 늘면 MINOR」)에 비추어 판단한다.

- **`tsc`가 고칠 자리를 전부 짚어 준다** — 037이 `Character` 유니온을 좁혔을 때와 같은
  방법이다. `VisionSetting`을 `"quick"` 하나로 좁히면 컴파일 오류가 대상 목록이 된다:
  - [src/diary/types.ts:48,51](../../src/diary/types.ts#L48-L51) — `VisionSetting` 유니온과
    `VISION_SETTINGS` 배열.
  - [src/app/resolve-generation.ts:88-95](../../src/app/resolve-generation.ts#L88-L95) — R5의
    `auto` 삼항. **`photoSignalPresent`는 남는다**(0장이면 엔진을 안 여는 판정은 유지).
  - [src/inference/on-device.ts:221](../../src/inference/on-device.ts#L221) — `depth` 선택 한 줄.
    `VisionDepth`([vision/types.ts:130](../../src/vision/types.ts#L130))와
    `IMAGE_TOKENS`도 함께 좁힐지 결정한다.
  - [src/ui/VisionPicker.tsx](../../src/ui/VisionPicker.tsx) — `OPTIONS`·`LABELS`. 선택지가
    하나면 이 컴포넌트와 설정 행 자체가 사라진다(설정 탭 「사진 보기」 섹션 제거).
  - [src/app/vision-setting-store.ts](../../src/app/vision-setting-store.ts) — `VisionPreference`,
    `isVisionSetting`, `loadVisionSetting`/`saveVisionSetting`. 파일 자체를 없앨지
    (설정이 없으면 저장할 것도 없다) 판단한다.
  - [src/schedule/task.ts:141](../../src/schedule/task.ts#L141) — 자동 생성 경로의 같은 분기.
  - `pipeline.ts`·`request.ts`·`wiring.ts`·`inference/select.ts`의 `vision: VisionSetting`
    시그니처 — 타입만 좁아지고 구조는 그대로일 가능성이 높다.

- **기존 사용자 데이터** — `preferences/vision-setting.json`에 `{"vision":"none"}`이나
  `{"vision":"detailed"}`가 이미 저장된 기기가 있다. `loadVisionSetting`이 **모르는 값을
  전부 `"auto"`로 떨어뜨리는 구조**라(파일 없음·깨짐·유니온 밖 전부 동일) 유니온만
  좁히면 옛 값이 자동으로 무시된다 — **마이그레이션 코드가 필요 없을 가능성이 높다.**
  다만 그 경로를 계약 테스트로 확인하고, 설정 파일 자체를 없앤다면 남은 파일을 지울지도
  정한다(008·037의 "앱이 사용자 저장물을 말없이 지우지 않는다" 계열).

- **범위 밖**:
  - **사진 0장인 하루에 VLM을 여는 것** — 위 결정 1. 011의 결론을 뒤집지 않는다.
  - **`VISION_PHOTO_LIMIT`(8) 조정** — 023이 시간 제약으로 정한 값이고 이 항목과 무관하다.
  - **`image_max_tokens` 두 값의 차이 실측(D6)** — `detailed`를 빼면 당장 필요 없다.
    나중에 깊이를 되살리고 싶어지면 그때 재는 것이 순서다(원칙 V — 안 잰 값을 쓰지 않는다).
  - **캡션 언어(영어)** — 011이 남긴 별건.

- **검증**:
  - `npm test` + `npm run lint`(`tsc`가 완료 조건 — 오류 0이 곧 이관 완료).
  - **Maestro `photo-vision.yml`이 깨진다** — `vision-none`·`vision-detailed`·`vision-auto`
    testID를 짚고 있다([.maestro/photo-vision.yml](../../.maestro/photo-vision.yml) 51~149행).
    `photo-selection-over-limit.yml`도 `vision-auto`·`vision-quick`을 쓴다. 둘 다 갱신 대상.
  - 실기기 dev 1회 — 사진 있는 하루에서 VLM이 실제로 돌고(`has_media=1`), 사진 0장인
    하루에서는 캡션 단계가 생략되는 것(011 회귀)을 `adb logcat`으로 확인한다.
  - 위반 주입: 설정에 `none`을 되살리거나 `auto` 분기를 되돌렸을 때 계약 테스트가 잡는지.

### 26. 프롬프트로 화자를 바꾼다 — 강아지·고양이 페르소나

- **출처**: `my-ollama` 커밋 `a39fae6` 「강아지·고양이 화자를 만들어 말끝과 습성을
  kanana 120런으로 잰다」(2026-09-09). 산출물은 `results/pet-speaker/PROMPT.md`(265줄)
  + `habit.json`·`smoke.json` + `scripts/pet-speaker/` 넷. **036이 이식한 컨셉 리포트
  (`results/concept-prompt/report.md`)와는 별개의 후속 연구다** — 그 리포트가 확정한
  E2SN 화자를 밑바탕으로 삼는다.
- **핵심**: 모델은 kanana(금동이) 그대로 두고 **프롬프트만으로 화자를 바꾼다.**
  E2SN의 뼈대(인식 구조 단서→짐작, 금지 조항, 일기 모양, 꼬리)는 한 글자도 안 바꾸고
  **바뀌는 자리가 다섯뿐**이다 — 정체 한 문장(휴대폰이되 *스스로를 강아지/고양이로
  여긴다*), 말끝 규칙 한 줄, 짐작 예시 어미, 제목 지시문, 그리고 **기록 블록의 고정
  문장 끝 글자**.
- **my-ollama 실측에서 배운 것 셋** (kanana 36런 + 습성 72런, 셀당 n=6~9 — 방향을
  가르기엔 충분하고 수치를 인용하기엔 부족하다고 리포트가 스스로 적었다):
  1. **말끝은 규칙 문장이 아니라 기록 블록의 어투가 정한다.** 머리가 한 글자도 안
     다른 두 안에서 기록 블록만 멍/냥으로 바꾸니 준수가 35~67% → **80~94%**로 뛴다.
     모델은 "무엇으로 끝내라"는 지시보다 **바로 위에 놓인 글이 어떻게 끝나는지**를
     따른다 — 036의 A-wrap(캡션을 캡션 자리에서 감싼다)과 같은 성질이다.
     **규칙을 더하지 말고 자리를 바꿔라.**
  2. **어미 예시를 나열하면 목록째로 베낀다.** v1이 일기 끝에 `있었멍: 카페, 마트,
     집`을 붙였다. 예시를 뺀 v2·v2b에서 기록 베끼기가 3~5/6 → 0~1/6으로 떨어졌다.
     036 §2.4의 "제목 지시문 반례를 78~94% 베낀다"와 같은 종류다.
  3. **습성은 감각(냄새·소리)이 아니라 상태(자고·깨고·기다림)로만 준다.** 감각을
     주면 화자에게 없는 감각을 주는 것이고 곧 **없는 단서를 만든다**. 그리고
     **`냄새와 소리로는 알지 못한다`는 부정문도 역효과**다 — 막으려고 이름을
     부르면 그 이름을 쓴다. 그 줄을 뺀 W3에서 감각 지어내기가 **0~1/9**다.
     **금지는 부정문 대신 좁은 허용 목록으로 쓴다**(`네가 느끼는 것은 …뿐이다`).
  - 가장 좋은 발견: **`낮잠`이 무지의 *이유*가 된다.** "자다 깨서 보니 벌써
    저녁이었멍"은 위트인 동시에 정직한 인식 고백이라 **컨셉과 원칙 II가 같은
    방향을 본다.** 게다가 습성은 `empty` 케이스에서만 5/6으로 나오는데, 지어낼 게
    없어 카페·마트를 만들던 바로 그 자리다.
- **헌법 v1.8.0 선행 개정 — ✅ 완료 (2026-09-17)**. 037·036·042와 같은 순서(헌법
  먼저, 코드는 다음 스펙). 지금 모양의 원칙 III이 이 작업을 **허용하지 않기**
  때문이다 — 페르소나가 화면에만 흐르고(프롬프트로 가는 것은 호칭 한 줄),
  캐릭터와 모델이 1:1이다.
  - **캐릭터의 단위를 페르소나로 옮겼다.** 제목 「모델은 캐릭터다」 → **「캐릭터는
    모델 위에 선다」**. 한 모델 위에 캐릭터가 여럿 설 수 있고(MAY), 같은 모델을
    쓰는 캐릭터끼리는 **모델 파일을 공유한다**(MUST) — 다운로드 1회로 셋이 열리므로
    「다섯 개를 다 받기」 금지와 오히려 맞는다. `Character` 유니온 하나·`roster.ts`
    단일 통과 지점·역방향 없음은 **그대로 유지**(저장소 소유자 결정).
  - **페르소나가 프롬프트를 바꿔도 된다**(MAY). 단 **그 차이도 실기기에서 관측돼야
    한다**(MUST) — 근거의 출처가 모델이냐 프롬프트냐만 다를 뿐, 관측 없이 붙인
    말투는 「지어낸 성격」이다. *(부수 발견: `prompt.ts`의 `E_TONE` 주석이 근거로
    든 "헌법 1.5.0 원칙 III 톤 조항"이 **개정 이력 서술에만 있고 조항 본문에는
    없었다.** 036이 이미 톤 줄을 프롬프트에 실었으므로 1.8.0이 그 자리를 채운
    셈이다.)*
  - **페르소나가 바꾸는 것은 말투와 태도뿐이다**(MUST). **정체·아는 범위·정직은
    원칙 I·II의 것이며 페르소나가 바꿀 수 없다**(MUST NOT). — *"신호를 보고 글을
    쓰는 것이 이 앱이고, 화자의 옷은 그 위에 입는다"*(저장소 소유자, 2026-09-17).
  - **진입 기준이 캐릭터마다 걸린다**(MUST) + **「말투를 걷어내고 판단한다」**(MUST).
    리포트가 스스로 경고한 자리다 — 장소 지어내기가 6/9로 v2b와 **똑같은데도**
    멍/냥 말투가 붙으면 읽는 쪽이 덜 아프게 느낀다(§6-1). 그대로 두면 진입 기준이
    **말투가 좋을수록 통과하기 쉬워지는** 방향으로 작동해, 재려던 것은 정직인데
    측정되는 것이 호감이 된다.
  - 용어 개정 **「씨앗」 → 「모델의 성질」**(저장소 소유자 지시 — "너무 추상적이다").
    헌법 살아 있는 문안 + `src/` 주석 4곳. **개정 이력과 지난 스펙 문서는 그때
    용어로 둔다.**
  - 원칙 II는 안 건드렸다 — 「화자가 뒤집히는 것은 위반이다」가 이미 반려동물
    미끄러짐(§6-4의 `참 착했멍`)을 덮는다.
- **구현은 별도 스펙(043 예정), 아직 열지 않았다.** 헌법은 길을 열 뿐이고 **실기기
  관측이 들여보낸다.** 스펙이 다룰 것:
  - **캐릭터 이름·정체 확정** — my-ollama는 뭉치(강아지)·나비(고양이)를 썼으나
    이름은 사람이 짓는다(원칙 III). 035의 사용자 작명과의 관계도 정리 대상.
  - **`persona.ts`·`prompt.ts`·`roster.ts` 구조** — `ASSETS`가 셋 다 `a1`을 가리키게
    되는 자리, 018의 `promptPrefix()` 바이트 동일성(캐릭터마다 접두사가 달라야
    한다 — P11)이 새 캐릭터 둘에서도 성립하는지.
  - **기록 블록의 말끝 치환** — `sentenceSignalLines()`가 낸 고정 문장만 바꾸고
    **캡션 본문은 원문 그대로 둔다**(011 P5 — 재서술은 VLM의 일이다).
- **넘어온 위험 — 리포트가 미측정으로 남긴 것들**:
  - **★ 캡션 케이스를 안 쟀다.** 리포트가 "제일 위험한 자리"라 스스로 적었고,
    `caption-ko`에서 습성이 안 나오고 **캡션 미끄러짐이 돌아온다**(강아지 3런 모두
    캡션을 옮겨 적고 캡션 속 인물을 주인의 관계로 만들었다). **042가 「사진 있으면
    반드시 VLM」으로 못 박은 지금, 이 저장소에서 캡션 케이스는 예외가 아니라
    주류다.** 043의 최우선 검증 대상.
  - **제목 퇴행 2~4/6** — 어느 변형에서도 그렇다. E2SN에 없던 퇴행이고 말끝 제약이
    제목 규칙의 자리를 밀어낸 것으로 보인다.
  - **말끝 준수 80~94%는 제품 기준으로 부족하다.** 리포트는 후처리(마지막 글자
    치환)를 "정직한 답"으로 제안하지만, 그건 판정 4갈래 바깥의 새 변환 단계이고
    원칙 I의 「생성된 그대로 저장한다」와 충돌을 검토해야 한다. **헌법은 이 갈래를
    열지도 닫지도 않았다** — 043의 결정 사항.
  - **어미가 깨진다** — v1에서 한 편 전체가 `멍`이 아니라 `멩`으로 나왔다(`아니었멩`,
    `있멩`). 한 글자 치환이 토크나이저 경계에서 흔들린다.
  - **2케이스 × 3런은 표본이 작다** — 리포트가 "6케이스 × 18런으로 다시 재야 한다"고
    적었다. 그 재측정을 my-ollama로 되돌릴지 043이 정한다.
