...
- [X] T020 **실기기 검증 완료**(2026-09-11, SM-S901N/Galaxy S22, dev).
  quickstart.md 2-1~2-5 전부 육안 확인:
  - **2-1(SC-001)**: 문구가 빈 상태에서 시작해 글자 단위로 채워짐,
    단계 전환 시 처음부터 재시작 확인.
  - **2-2(SC-002)**: 완료 후 다음 전환까지 완성된 문구가 그대로 머무름,
    별도 대기 표현 없음 확인.
  - **2-3(SC-005)**: 타이핑 도중 화면 탭 → 무반응, 그만두기는 정상
    동작 확인.
  - **2-4(SC-003)**: 결과 화면 전환 시 애니메이션 잔상 없음 확인.
  - **2-5(SC-004)**: 회전 표시·문구·그만두기 구성 무변경, 새 정보
    없음 확인.
  - **2-6 Maestro — T017 예측이 실측으로 확인됨, 흐름 수정 완료**:
    18흐름 전체 실행에서 `writing-monologue.yml`·
    `writing-monologue-expansion.yml` 둘 다 `runFlow: when: visible:
    "쓰고 있다"` 블록이 **SKIPPED로 관측됨**(추측이 아니라 실측) —
    독백 검증(FR-001·003·004·009·010·013·014) 전체가 한 건도 실행되지
    않은 채로 흐름 자체는 실패로 안 잡히는 상태였다. `when` 조건을
    타이핑과 무관하게 즉시 렌더되는 "그만두기" 버튼으로 교체해 두
    파일 모두 수정.
  - **수정 후 재검증 — quiet 캐릭터의 구조적 한계로 완전한 재현은
    안 됨, 037과 무관한 새 발견 아님**: `.maestro/writing-monologue.yml`
    을 `maestro test`로 3회 단독 재실행. 실제로는 생성 자체가 성공
    (09-11 일기 저장 확인)했지만, **quiet(kanana) 콜드/웜 모두 생성이
    매우 빨라 `tapOn` 직후 4개 `assertNotVisible` 체크 + `when` 평가
    시점보다도 먼저 끝나** "그만두기" 버튼이 뜨는 순간조차 Maestro가
    못 잡고 블록이 계속 SKIPPED됐다(logcat으로 TAP 이벤트 이후 매
    회 목록 화면 또는 저장 완료 상태로 바로 전환된 것을 확인). **이건
    039의 회귀가 아니라 037(로스터를 kanana 하나로 축소)이 낳은
    구조적 제약이다** — narrative(루이)처럼 느린 캐릭터가 로스터에
    없어, 이 파일 자체의 기존 주석("quiet처럼 빠른 캐릭터는 독백이
    스쳐 지나가 SKIPPED가 될 수 있다. 그것은 실패가 아니다")이 정확히
    예견한 상황이 037 이후 사실상 **유일한 경로**가 됐다. 코드 레벨
    수정(`when` 조건 교체)은 038 원칙(추측으로 고치지 않고 실측 후
    최소 수정)에 따라 완료했고, 남은 SKIPPED는 로스터 제약이지 흐름
    결함이 아니므로 추가 수정하지 않는다.
  - Maestro 18흐름 전체 재실행에서 나머지 실패 9개는 전부 039와 무관:
    캐릭터 이름이 "금동이"→"오구삼춘"으로 사용자 지정됨(035, 이전
    세션 잔여 상태), 037의 알려진 로스터-하나 구조적 실패
    (`download-conflict`·`parallel-model-download`), `unified-
    permission-onboarding.yml`의 `pm clear`가 앱 데이터(모델 포함)를
    초기화해 그 뒤 5개 흐름이 "일기"/"설정"/"개발자" 탭을 못 찾음
    (024·038이 이미 기록한 알려진 상호오염). 검증 과정에서 지워진
    모델을 온보딩에서 재다운로드해 복구했다.
  - 테스트용으로 생성된 2026-09-11 일기는 검증 종료 후 삭제해 기기
    상태를 정리했다.
