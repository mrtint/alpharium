...
- [X] T024 (부분 확인, 2026-08-23) quickstart.md B1~B6를 실기기(dev 빌드,
  SM-G986N, 루이/narrative)에서 시도했다.
  - **직접 관측된 것**: 신호 확인 단계의 신설 문구 다수(예: "하루의 자취를
    따라가보는 중…", "기록을 차근차근 살펴보는 중…")가 실제로 뜬다.
    일기가 정상 생성되고 저장된다(제목·본문·"이전 일기를 덮어썼다" 표시
    포함). 덮어쓰기 확인 화면(012)이 정상 동작한다. 판정 거부(원칙 I)도
    한 차례 관측됐다(재시도로 통과). 숫자·퍼센트가 화면에 없다(FR-004).
  - **직접 관측하지 못한 것 — B1·B2(콜드/핫 로드 문구)**: 이 세션의
    기기 상태(사진 3장, 파일시스템 캐시가 따뜻함)에서는 신호 확인→사진
    보기→모델 로드→글쓰기 전체가 매 시도마다 수 초 내로 끝나, 화면
    스크린샷 연속 캡처(간격 1~2초)와 Maestro `extendedWaitUntil` 폴링
    양쪽 모두로 로드 단계 문구를 붙잡지 못했다(총 6회 이상 시도, 앱
    강제 재시작으로 콜드 스타트를 재현했음에도 동일). 로드맵의 "콜드
    최대 242초"는 최악값이며 이번엔 그보다 훨씬 짧았던 것으로 보인다.
    B3(장수 많음)·B5(로드 실패)·B6(로드 도중 취소)도 이번 세션에서는
    시도하지 않았다(quickstart.md가 이미 문서화한 대로 별도 조건
    — 사진 5장 이상, 모델 삭제, 정확한 취소 타이밍 — 이 필요하다).
  - **코드 근거**: `pickMonologue(stage, branch, ...)`가 `stage==="load"`
    조합에서 실제로 문구를 반환하는 것은 기기 없는 테스트(`monologue.
    test.ts`, 1437/1437 통과)가 직접 검증했고, `DiaryHomeScreen.tsx`의
    `onProgress` 배선도 `diary-home.test.tsx`가 검증한다 — 화면에 "뜰
    문구가 있다"는 코드 수준에서 확정됐지만, 이 세션에서는 그 순간을
    육안으로 붙잡지 못했다는 뜻이다(원칙 V — 관측한 것과 관측하지 못한
    것을 구분해 남긴다).
  - Maestro 흐름(`.maestro/writing-monologue-expansion.yml`)은 "일기
    쓰기" 버튼이 화면 스크롤 아래에 있어 스크롤 없이 도는 원본 상태로는
    015의 기존 흐름(`writing-monologue.yml`)과 함께 이 세션에서
    실패했다 — 016이 만든 회귀가 아니라(015 흐름도 동일하게 실패)
    화면에 누적된 콘텐츠(캐릭터 5개·과거 하루 3개)로 버튼 위치가
    밀린 것으로 보인다. 016 범위 밖의 별도 사안으로 남겨 둔다(원본
    파일은 이 세션에서 수정하지 않았다).
