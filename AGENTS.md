...
- **★ D5(백그라운드 자동 생성)는 막혔고, 막은 것은 042가 아니다** — 020의
  `decision.ts`가 `isNearTarget(hour, targetHour)`로 거르는데 목표 12시의 창이
  `[12,15)`이고 세션 시각이 17시였다. 개발자 탭 트리거가 **`결과: skipped`**를 화면에
  표시했고 일기는 쓰이지 않았다. `cmd jobscheduler run -f`도 같은 `runAutoDiaryTask()`를
  거친다. **`targetHour`를 바꾸지 않았다**(계속 쓰는 설정). **12:00~14:59에 재시도해야
  한다** — 그때 `files/diary/`의 새 파일과 `has_media=1`을 **함께** 본다
  (`skipped`는 로그만으로 정상 완주와 구분되지 않는다, 027 §1).
- **건너뛴 실기기 테스트는 통과가 아니다**(원칙 V) — D5는 미확인으로 남는다.
