...
- **① 다크 모드 dimmed** — `app.json`의 `userInterfaceStyle: "light"`가 Android에서 무시된다(`@expo/prebuild-config`의 `withAndroidUserInterfaceStyle`가 `expo-system-ui` 미설치 시 경고만 내고 아무것도 안 함, 설치본 직접 확인). 그래서 시스템 다크 모드가 켜지면 One UI가 흰 배경 RN 뷰에 force-dark를 씌운다. **수정: `expo-system-ui` 설치**(→ `userInterfaceStyle: "light"`가 실제 적용) **+ 새 config plugin으로 Android 테마에 `android:forceDarkAllowed="false"` 선언**(→ One UI 8.5의 공격적 force-dark 이중 차단). `styles.xml`의 `AppTheme` 부모가 `Theme.AppCompat.DayNight.NoActionBar`인 것도 확인됨 — plugin이 이 테마에 항목을 더한다.
