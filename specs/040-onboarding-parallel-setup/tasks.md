...
- [X] T016 [US1] 기존 021 `.maestro/unified-permission-onboarding.yml`이
      이번 변경(스텝 자동 전환)으로 깨지는지 실기기에서 확인하고, 깨졌다면
      흐름을 새 자동 전환 타이밍에 맞게 수정. **완료**(2026-09-11,
      SM-S901N): M1~M4가 전부 통과해 **수정이 필요 없었다** — 자동 전환
      타이머(1.5초)가 Maestro의 기본 대기와 충돌하지 않는다. M5 실패는
      040과 무관한 기기 상태 문제다(T033 참조).
...
- [X] T026 [US3] 실기기에서 US3 Independent Test 수행 — liveness 통과 후
      ...
      홈 화면에 반영되는 것을 재확인했다.
- [X] T027 [US3] 실기기에서 liveness 실패 케이스(모델 파일 일부만 배치 등
      가능한 방법으로 유도) 확인 — 035 기존 실패 안내가 뜨고 자동 생성이
      시도되지 않는지(quickstart.md 9번, FR-008a). **시도했으나 이 경로로는
      유도되지 않는다**(2026-09-11): a1을 50MB로 자르니 앱이 **로드를
      시도하기 전에 파일 크기 검증에서 먼저 걸러내** 재다운로드로 넘어갔다
      — liveness까지 도달하지 않는다. 실패 갈래는 035의 계약 테스트(L3 네
      갈래)로 갈음한다. **부수 소득**: 이 과정에서 FR-006 대기 화면("잠시만요
      / 일기를 쓰는 데 필요한 것을 마저 내려받는 중이에요" + 진행률,
      그만두기 버튼 없음)을 관측했다 — 그때까지 미확인이던 항목이다.
...
- [X] T032 `scripts/run-device-tests.mjs`의 `FLOWS`에
      ...
      AGENTS.md 경고)
- [X] T033 기존 회귀 흐름 재확인 — `.maestro/unified-permission-
      onboarding.yml`(021)과 `.maestro/photo-vision.yml` 등 035/029 관련
      기존 흐름이 이번 변경 후에도 PASS하는지 실기기에서 1회 확인.
      **부분 완료**(2026-09-11): 021 흐름의 M1~M4(온보딩 우선 표시, 모델
      정보 비노출, 단계 순차, 건너뛰기 반복)는 **전부 통과** — 040의 자동
      전환 변경이 021 흐름을 깨지 않았다. M5("에셋 미준비로 재실행하면
      온보딩이 다시 뜬다", 029 FR-020)만 실패했는데 **040 회귀가 아니다**
      — 이 흐름 주석 124행이 스스로 명시한 예외("에셋을 미리 받아 둔
      기기에서는 이 assert가 틀리므로 반드시 clearState 상태에서 돌린다")에
      해당한다. 이 세션은 a1 OOM 우회를 위해 모델 3개를 백업본에서 복구해
      둔 상태라 전제가 깨졌다. **모델이 없는 기기에서 재확인이 필요하다.**
