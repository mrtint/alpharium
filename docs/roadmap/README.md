...
- [ ] **사진이 있는 하루는 VLM을 반드시 거친다** (사진 보기 설정에서 「보지 않음」·「자세히 봄」을 없앤다 — 사진이 1장 이상이면 VLM이 무조건 돌고 사용자가 끌 수 없다. 0장인 하루는 011대로 엔진을 열지 않는다. `detailed`는 같은 모델의 `image_max_tokens` 256 vs 1024 차이뿐이고 그 차이를 잰 적이 없다. **헌법 「사진과 시각 처리」 개정이 선행**한다 — 그 절이 지금 세 갈래 선택을 명시하고 있다. 아래 25번 상세)
