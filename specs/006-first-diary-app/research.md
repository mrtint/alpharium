...
| 값 | 지금 | 성질 |
| --- | --- | --- |
| release에서 동적 import가 도는가 | **돈다** | **실측 2026-08-20** — 저장·조회·생성이 전부 동작 |
...
| 프롬프트 교정 뒤 단언이 사라지는가 | **캐릭터에 달렸다** | **실측 2026-08-20.** `quiet`는 0건, `imaginative`는 남았다 |
| 남은 단언이 프롬프트 탓인가 모델 탓인가 | **모델의 성질** | **실측 2026-08-20** — 같은 프롬프트·같은 하루로 갈렸다 |
| release에서 `llama.rn`이 도는가 | **돈다** | **실측 2026-08-20** — ProGuard 켠 채로 모델 적재·생성 성공 |
| release가 `prod`로 판정되는가 | **된다** | **실측 2026-08-20** — `NODE_ENV=production`을 직접 줬다. 진단 화면 없고 build-error 아님 |
| APK 크기 | **172MB** | **실측 2026-08-20.** ABI 3개(arm64-v8a·armeabi-v7a·x86). `librnllama.so`만 9.8MB |
| 덮어 설치에서 일기가 보존되는가 | **보존된다** | **실측 2026-08-20** — debug APK 위에 release를 `adb install -r`로 덮었고 일기가 남았다 |
| 기기 재부팅 뒤에도 일기가 남는가 | **남는다** | **실측 2026-08-20** — 목록에 뜨고 눌러서 읽힌다 |
