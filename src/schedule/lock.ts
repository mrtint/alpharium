 ...
 * 규칙은 **그때 로스터에서 가장 느린 캐릭터**의 백그라운드 완주 실측 최댓값
 * M초 × 2, 분 단위 올림이다(contracts/stale-lock-basis.md SL4,
 * data-model.md §5). 019가 `quiet` 최장 완주 2분 27초(147초) × 2 = 294초를
 * 5분(300초)으로 올린 것과 같은 방식이다.
 *
 * **024 실측**(2026-08-30, SM-S901N/Galaxy S22, Android 16, debug):
 * `narrative`(exaone) 사진 있는 날(08-28, 캡션 8장) 완주 벽시계 ≈ **170초**
 * (VLM 캡션 `visionMs` 73.0초 + 캐릭터 모델 `writingMs` 89.8초 + 적재).
 * `M ≈ 170초` → `ceil(170 × 2 / 60) × 60 = 360초 = 6분`.
 * 상세는 findings.md §1.
 *
 * ─────────────────────────────────────────────────────────────────────────
 * ★ 037(헌법 1.6.0) — **근거로 든 `narrative`가 로스터를 나갔다.** 자동 생성
 * 부적합이 실측으로 확정됐다(024 T034 헤드리스 완주 불가).
 *
 * **그래도 값 6분은 그대로 둔다.** 줄이려면 "이제 가장 느린 것은 `quiet`이고
 * 그 완주는 N초"를 다시 재야 하는데, **안 잰 값을 쓰는 것이 원칙 V
 * 위반이다.** 6분은 `quiet`에 대해 넉넉하다 — 024 헤드리스 완주
 * `writingMs` 52.5초, 036 포그라운드 19~22초, 037 포그라운드 24.7~31.6초.
 *
 * 위 170초는 **여전히 유효한 관측**이므로 지우지 않는다. 지금은 "그때 가장
 * 느린 캐릭터가 그만큼 걸렸고, 그 값으로 정한 상한이 지금 캐릭터에게 넉넉하다"
 * 로 읽는다. 로스터가 늘어 더 느린 캐릭터가 들어오면 그때 다시 잰다.
 * ─────────────────────────────────────────────────────────────────────────
 *
