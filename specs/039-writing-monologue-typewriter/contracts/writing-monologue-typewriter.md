...
- **C4**: `key`는 렌더되는 문구 문자열 자체(`screen.line ?? "쓰고
  ...
  다시 타이핑한다(FR-002, data-model.md 상태 다이어그램). **실측
  (구현 세션, 위반 주입 T013)**: `TypewriterText`(038)가 이미 내부적으로
  `key={props.text}`를 갖고 있어(`TypewriterText` 함수 본문), 호출부가
  외부 `key`를 생략해도 `text` prop이 바뀌면 리마운트가 그대로 일어난다
  — 이 외부 `key`는 방어적 명시일 뿐 제거해도 테스트가 못 잡는 무해한
  중복이다. 가독성을 위해 계속 명시하되, 이 계약이 "위반 주입으로
  검증 가능"하다고 오인하지 않는다.
