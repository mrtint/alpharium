...
**⚖️ 표시된 테스트는 짝이 되는 구현보다 먼저 쓴다.** 문서상의 번호 순서가 작업 순서가
아니다 — 각 구현 과제에 **선행 게이트**를 적어 두었다. 게이트가 가리키는 테스트를
작성하고 **실패를 확인한 뒤에** 구현을 시작한다.

| 구현 | 선행 테스트 |
| --- | --- |
| T005 관측 여부 표현 | T006 |
| T008 퍼소나 엔티티 / T008a 부여 / T008b 저장 | T009 |
| T012 기록 묶음 | T013 |
| T015 화자 판정기 | T016 |
| T023 권한 거부 | T024 |
| T025 집계 산출기 | T028 |
...
- [ ] T033 [US1] ⚖️ 원칙 IV `AIEngine` 테스트를 `__tests__/contract/engine.test.ts`에 작성 — **정상·실패·형식 불량 응답** 셋 모두 (헌법 원칙 IV 명시 대상). **추론 축이 입력의 쓸모를 재판정하지 않음을 함께 검사** — 「적음」 판정이 붙은 집계와 「보통」 집계가 같은 경로로 처리된다 (001 FR-013b, 003 FR-282)
...
- **⚖️ 테스트는 짝 구현보다 먼저** — 「Tests」 절의 표에 있는 열두 쌍. 헌법 원칙 IV(MUST)이며
  선택이 아니다. 문서상의 번호 순서가 작업 순서가 아니다
