...
**L9. 권한을 영구 거부("다시 묻지 않음")한 뒤 설정을 다시 켜도 새 상태를
만들지 않는다.** Android가 권한 대화상자 자체를 다시 띄우지 않는 상황에서도,
그 이후의 `reverseGeocode()` 호출은 L8과 동일하게 `unknown`으로 귀결될 뿐이다
— "영구 거부"를 구분하는 별도 갈래를 만들지 않는다(L5의 확장, 원칙 IV 판정
갈래 최소화와 같은 정신). **설정 토글 자체는 사용자가 낸 값(켜짐)을 그대로
유지한다** — 권한이 없다고 앱이 토글을 임의로 꺼서 사용자의 선택을 되돌리지
않는다(MUST NOT).
