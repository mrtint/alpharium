...
- [X] T022 quickstart.md D1을 실행한다 — `expo prebuild --clean` 후 새 권한이
      필요 없음을 `adb shell dumpsys package`로 확인한다. **통과** — 권한 목록이
      013 이전과 정확히 같은 둘(READ_MEDIA_IMAGES·ACCESS_MEDIA_LOCATION)뿐이다
- [X] T023 quickstart.md D2를 실행한다 — 실사 사진이 있는 하루로 캡션 시간을
      재고, IMAGE 청크가 7~9개에서 1개로 줄었는지, 총 시간이 129초의 절반
      이하인지 확인한다. **결과를 AGENTS.md에 실측으로 기록한다**(FR-021).
      **통과** — 캡션 129초 → 23초(82% 감소), IMAGE 청크 9 → 1. 실기기 검증
      중 두 개의 조용한 실패(URI 계약 불일치, 입력·출력 양쪽)를 발견해 고쳤다 —
      AGENTS.md 013 절 참조
- [X] T024 quickstart.md D3을 실행한다 — 같은 실행에서 나온 일기와 원본 사진을
      대조해 새로 지어낸 내용이 없는지 확인한다(SC-002). **통과** — "공원에서
      피크닉", "큰 우산이 그늘을 만들어주고, 대화하는 모습"이 원본 사진과
      정확히 일치했다
- [ ] T025 quickstart.md D4를 실행한다 — 010의 `seed:day`로 손상된 이미지를
      섞은 하루에서 나머지 사진의 캡션이 살아있는지 확인한다(SC-005). **기기
      없는 테스트로만 검증됨**(resize.test.ts·caption.test.ts) — 실기기 미확인
- [ ] T026 quickstart.md D5를 실행한다 — 세 번 생성 후 `vision-cache/` 파일 수,
      갤러리 미노출, 원본 사진 무결성을 확인한다(SC-003·004·007). **기기 없는
      테스트로만 검증됨** — 실기기 미확인
- [ ] T027 quickstart.md D6을 실행한다 — 방향 정보가 있는 세로 사진으로 캡션을
      돌려 방향이 뒤집히지 않는지 확인한다. **간접 증거만 있다** — D2·D3
      검증에 세로 사진이 섞여 있었고 캡션에 방향 이상 서술이 없었으나, 명시적
      검증은 아니다
- [X] T028 `npm test`(전체)와 `npm run lint`가 모두 통과하는지 최종 확인한다.
      **통과** — 1276개 테스트, 헌법 검사 위반 0건
- [X] T029 AGENTS.md에 013의 결과를 정리해 기록한다 — 실측 시간, 실기기 확인
      완료 여부, 남은 미확인 사항(있다면). **완료** — 「013 — 사진을 보기 전에
      줄인다」 절 참조
