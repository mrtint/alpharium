...
/**
 * 사진을 어떻게 다룰지. **이제 고를 것이 없다**(042, 헌법 v1.7.0 「사진과 시각 처리」).
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * **★ 값이 하나인 것이 이 타입의 요점이다.**
 *
 * 헌법 v1.7.0이 못 박았다 — 「사진 접근이 허용되어 있으면 그 하루의 사진은 반드시
 * 본다(MUST). 사용자가 이것을 끄는 경로를 두지 않는다(MUST NOT). 보는 깊이는 하나로
 * 고정한다(MUST).」
 *
 * 그래서 `"none"`(사용자가 끄는 길)과 `"detailed"`(재지 않은 깊이 다이얼)가 나갔다.
 * **되살릴 길을 조항으로 남기지 않기로 했다**(저장소 소유자 결정, 2026-09-14) —
 * 037이 로스터에 "하나인 것은 현재 상태다"를 남긴 것과 **다른 선택**이다. 재서
 * 근거가 생기면 그때 헌법을 다시 고친다.
 *
 * **「볼 것이 없으면 열지 않는다」는 이 타입이 담지 않는다** — 그것은 설정이 아니라
 * 그 하루의 사진 신호가 정하는 것이고(011), 판정은 파이프라인이 한다.
 *
 * ⚠️ **유니온을 다시 넓히는 것은 `tsc`가 못 잡는다**(타입 오류가 아니다).
 * `__tests__/vision/photo-vision-always.test.ts`의 C1이 유일한 방어다.
 * ─────────────────────────────────────────────────────────────────────────────
 */
export type VisionSetting = "quick";

/** 시각 설정 전체. 순회와 검증에 쓴다. **하나인 것이 정상이다**(037의 `CHARACTERS`와 같다) */
export const VISION_SETTINGS: readonly VisionSetting[] = ["quick"] as const;
