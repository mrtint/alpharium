...
> - **§2 배터리 예외/무예외 소크 — 스펙 027로 이관, 2026-09-14에 종결**.
>   **예외 소크(SC-003)는 027 US1이 충족**(유효 라운드 1회, 목표 시각
>   +13분). **무예외 24h 소크(SC-004/SC-002)는 접었고 미판정으로 남는다**
>   (저장소 소유자 결정 — 24시간 기기 사용 포기가 대가이고 019가 같은
>   조건을 이미 실측). 아래 §2 표 참조.
...
|---|---|---|---|---|---|---|---|---|
| true | 12 | 2026-09-14 11:59:03 | 12:12:33 | **13** | 5 | +14m59s983ms | false | **027 US1 유효 라운드**(SM-S901N). 완주 81초, `writingMs` 69.4초, `2026-09-14.json` 저장·알림 발송. 상세는 027 `findings.md` §1. |
| _(접음 — 미판정)_ false | — | — | — | — | — | — | — | **027 US2를 수행하지 않기로 결정**(2026-09-14). 019가 같은 조건에서 관측한 19시간 33분 간격이 유일한 참고값이나 **다른 세션 값이라 이 칸에 쓰지 않는다**(원칙 V). |

**MUST 판정**:
- [X] **SC-003 (예외): 목표 시각으로부터 첫 시도 ≤ 60분 — 충족**
  (2026-09-14, 027 US1). 유효 라운드 1회에서 **13분**.
- [ ] SC-002·SC-004 (무예외): 목표 시각 후 24시간 안 ≥ 1회 시도 —
  **미판정으로 남는다**(027 US2 접음, 2026-09-14).

**SHOULD**: 예외 시 3회 이상 표본의 과반 ≤ 40분 — _(표본 1회라 미충족.
원시값 13분. 019의 10분·32분은 다른 세션이라 합쳐 세지 않는다.)_

**★ 소크 라운드를 무효로 만드는 조건이 둘이다**(027 US1이 실측으로 분리).
`screenTouchedDuringRound` 한 칸이 둘을 함께 담고 있었다:
(1) Doze가 깨진다, (2) **앱이 전경으로 돌아오면 `expo-background-task`가
태스크를 아예 실행하지 않는다**(`runTasks: App is in the foreground`).
후자의 `inForeground`는 **Activity의 start/stop에 반응하지 화면 on/off가
아니다** — 화면을 꺼도 `wm_on_stop_called`이 뒤따르지 않으면 안 돈다.
`dumpsys`의 `ResumedActivity`는 화면이 꺼진 뒤에도 스테일하므로 판정
근거로 쓰지 않는다. 상세는 027 `findings.md` §1.
...
- **T012·T013·T014 / T032·T033 — §2 배터리 예외/무예외 소크** — **스펙
  027로 이관 후 2026-09-14 종결.** 예외 소크(SC-003)는 027 US1이 **충족**
  (목표 시각 +13분), 무예외 24h 소크(SC-004)는 027 US2를 **접어 미판정**.
  원래 사유: 자연 15분+ 주기·24h 비동기 대기가 필요해 별도 실기기 세션에서
