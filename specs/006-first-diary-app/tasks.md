...
- [X] T038 [US4] 서명 키를 만든다 (`keytool -genkeypair`). **저장소 밖에 백업한다** — 잃으면 덮어 설치가 영영 끊긴다
...
- [X] T054 [P] [US6] `__tests__/diary/prompt.test.ts`에 검사를 더한다: 단언 금지 지시가 있고 **짐작 허용도 함께** 있다 (FR-036·037)
- [X] T055 [P] [US6] `__tests__/diary/acceptance.test.ts`의 「갈래가 넷」 검사가 그대로 통과하는지 확인한다 (FR-039, SC-021)

### Implementation

- [X] T056 [US6] `src/diary/prompt.ts`의 `SPEAKER_RULES` 문안을 고친다 — 005에서 "날씨가 정말 좋았다"·"친구와 산책을 했다"가 새어 나온 자리. **`prompt.ts` 안에서만 고친다**(FR-038), **캐릭터별로 다르게 하지 않는다**(FR-040), **판정 갈래를 늘리지 않는다**(FR-039)
- [X] T057 [US6] `instructionLines()`가 고친 문안과 **같은 상수에서** 나오는지 확인한다 — 어긋나면 되뱉기 판정이 조용히 무력해진다 (005 P-7)
...
- [X] T060 D1: 앱이 뜬다 — `Unable to load script`가 없다 (SC-002)
- [X] T061 D2·D3: 캐릭터를 고르고 모델을 받고 일기를 쓴다 (SC-001)
- [X] T062 D4: 「쓰고 있다」뿐이고 진행률·시간·생성 중인 글이 없다 (SC-020)
- [X] T063 D5·D6: 목록에 방금 쓴 일기가 있고 눌러서 읽는다 (SC-012)
- [X] T064 **D7: 앱을 완전히 종료하고 다시 연다 — 일기가 그대로 있다** (SC-008)
- [X] T065 **D8: 기기를 재부팅하고 앱을 연다 — 일기가 그대로 있다** (SC-008)
- [X] T066 **D9: 다른 날짜로 하나 더 쓴다 — 둘 다 목록에 있다** (SC-008a)
- [X] T067 D10: 진단 화면이 어디에도 없다 (SC-010)
- [X] T068 D11: **사람이 일기를 읽는다** — 화자가 휴대폰이고 기록에 없는 것을 단언하지 않았다. 짐작은 위반이 아니다 (SC-017·018)
...
- [X] T071 실기기에서 관측한 것을 `AGENTS.md`의 「이전 작업에서 실측으로 확인된 것」에 적는다 — **날짜·기기·빌드 종류(release)를 함께** 남긴다 (원칙 V)
- [X] T072 `research.md` §7의 「짐작으로 남는 것」 표를 관측 결과로 갱신한다 — 확인된 것은 사실로, 여전히 모르는 것은 미확인으로
