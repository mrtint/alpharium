...
import type { HomeMenuItem } from "./src/ui/HomeMenu";
...
function AppFrame() {
   ...
   * 아니라 항목 자체가 없다(024 FR-024·SC-013을 그대로 잇는다).
   */
  const menuItems = useMemo<readonly HomeMenuItem[]>(
    () => [
      { key: "settings", label: "설정", onPress: () => setRoute("settings") },
      ...(showsDiagnostics
        ? [{ key: "developer", label: "개발자", onPress: () => setRoute("developer") }]
        : []),
    ],
    [showsDiagnostics],
  );
