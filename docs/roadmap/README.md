...
>
> **진행 전 항목은 스펙킷 번호와 의존성·순서가 전혀 없다**: 스펙킷 번호(`specs/NNN-*`)는 스펙을 실제로 여는 순간에만 매겨진다. 아직 진행하지 않은 로드맵 항목에는 "047 예정" 같은 번호도, "4차·5차" 같은 차수도, 다른 항목 뒤에 와야 한다는 순서도 적지 않는다 — 어느 항목을 언제 열지는 사용자가 정한다. 항목은 로드맵 번호(예: 31번)로만 가리키며 그 번호는 식별자일 뿐 순서가 아니다. 스펙이 열린 뒤에만 그 스펙 번호를 기록한다(046·047에서 선점 번호를 두 번 밀어야 했던 것이 경위).
...
- [x] **Modernist 디자인 시스템 적용 1차 — 스플래시·권한 요청 흐름** (043 — 브랜드 스플래시 및 자동 순차 권한 요청 흐름 재구현, 색 토큰 Modernist 전면 교체. 043 구현 완료. 아래 27번 상세)
- [x] **Modernist 디자인 시스템 적용 2차 — 첫 만남 및 캐릭터 작명** (044 — `WelcomeScreen` Modernist 레이아웃 이관, 035 작명 규칙 및 040 백그라운드 병렬 다운로드 트리거 계약 준수. 아래 28번 상세)
- [x] **Modernist 디자인 시스템 적용 3차 — 필수 에셋(VLM & LLM) 다운로드 전용 화면** (045 — 기존 단순 스피너 `WaitingForDownloadScreen`을 대체하는 독립된 Modernist 에셋 다운로드 화면. 원칙 III·IV 준수, 시각 모델 및 언어 모델 준비 상태 안내. 아래 29번 상세)
- [x] **다운로드 진행 화면(캐러셀 + 프로그레스 바) 완성** (046 — 045가 남긴 정적 슬라이드·정적 진행 바를 실제로 동작하게 완성. `react-native-reanimated-carousel`(+`react-native-gesture-handler` 신규 도입)로 무한 순환 캐러셀(스와이프+자동 전환 병행), 4분할 애니메이션 프로그레스 바가 실제 합산 진행률(`essentialDownloadFraction()`)을 반영, 실패해도 화면 전환 없이 안내 문구만 교체 + 10초 자동 재시도. Modernist 시리즈 번호 밖(29번과 31번 사이). 아래 30번 상세)
- [ ] **Modernist 디자인 시스템 적용 — 일기 목록 및 쓰기 진입** (스펙 미착수 — `DiaryListScreen` + `DayPicker`. 상단 날짜 선택 바, 일기 쓰기 버튼 영역, 일기 카드 목록, 빈 상태 뷰. 아래 31번 상세)
- [ ] **Modernist 디자인 시스템 적용 — 일기 작성 진행 연출 및 보조 화면** (스펙 미착수 — `DiaryHomeScreen`의 `writing` 독백 타자기 연출 및 회전 인디케이터, `OverwriteConfirmScreen`, `BuildErrorScreen`. 038 타자기 및 독백 갱신 계약 엄격 보호. 아래 32번 상세)
- [ ] **Modernist 디자인 시스템 적용 — 일기 상세 본문 및 관측 신호** (스펙 미착수 — `DiaryDetailScreen` 본문 Modernist 타이포그래피, 038 생성 직후 타자기 첫 노출 연출(`reveal`), 017 "이 일기가 본 것" 메타데이터 및 소요 시간 표기. 아래 33번 상세)
- [ ] **Modernist 디자인 시스템 적용 — 사진 슬라이더 및 풀스크린 갤러리 모달** (스펙 미착수 — `DiaryDetailScreen` 내 025 갤러리 계약 단독 격리. 코어 RN 가로 슬라이더 + 풀스크린 모달 갤러리 뷰어. 아래 34번 상세)
- [ ] **Modernist 디자인 시스템 적용 — 전역 하단 탭 바 & 설정 기본 섹션** (스펙 미착수 — Modernist 전역 하단 탭 바, `AuthorPicker`, 일반 설정 토글(`Geocoding`, `AutoDiary`). 아래 35번 상세)
- [ ] **Modernist 디자인 시스템 적용 — 설정 탭 모델 관리 및 권한 관리** (스펙 미착수 — `CharacterListScreen` / `ModelSection` 모델 다운로드/삭제 관리, `PermissionsSection` 권한 상태 카드 및 온보딩 재실행. 아래 36번 상세)
- [~] **프롬프트로 화자를 바꾼다 — 강아지·고양이 페르소나** (`my-ollama` 후속 연구 `results/pet-speaker/`(커밋 `a39fae6`, kanana 120런) 이식. 모델은 kanana 그대로 두고 **프롬프트만으로 화자를 바꾼다** — 036이 이식한 E2SN의 뼈대·인식 구조·금지 조항을 유지하고 말투 층만 갈아 끼운다. **헌법 v1.8.0 선행 개정 완료**(2026-09-17): 페르소나가 프롬프트에서 말투·태도를 지시해도 되고(MAY), 한 모델 위에 캐릭터가 여럿 설 수 있다(MAY). 용어 「씨앗」→「모델의 성질」. **구현은 별도 스펙(스펙 미착수)** — 진입 기준은 캐릭터마다 실기기 관측이라 my-ollama 120런은 후보를 좁힌 것이지 근거가 아니다. 아래 26번 상세)
...
- **구현은 별도 스펙(스펙 미착수), 아직 열지 않았다.** 헌법은 길을 열 뿐이고 **실기기
  관측이 들여보낸다.** 스펙이 다룰 것:
...
- **넘어온 위험 — 리포트가 미측정으로 남긴 것들**:
  ...
  - **말끝 준수 80~94%는 제품 기준으로 부족하다.** 리포트는 후처리(마지막 글자
    ...
    열지도 닫지도 않았다** — 그 스펙의 결정 사항.
  - **어미가 깨진다** — v1에서 한 편 전체가 `멍`이 아니라 `멩`으로 나왔다(`아니었멩`,
    `있멩`). 한 글자 치환이 토크나이저 경계에서 흔들린다.
  - **2케이스 × 3런은 표본이 작다** — 리포트가 "6케이스 × 18런으로 다시 재야 한다"고
    적었다. 그 재측정을 my-ollama로 되돌릴지 그 스펙이 정한다.

### Modernist 디자인 리뷰 보드 — 참조 위치와 화면 ID 지도

27~29번·31~36번(043~045 및 아직 스펙이 열리지 않은 항목) 전부가 참조하는 디자인 리뷰 보드 원본은
저장소 밖에 있다 — (30번은 없음, 아래 별도 절의 046이 그 자리를 차지했으나
Modernist 시리즈 밖이라 리뷰 보드를 참조하지 않는다)
**`C:\Users\mrtin\Downloads\Alpharium Mobile Screens (standalone).html`**
(claude.ai/design 산출물, 저장소에 커밋하지 않음). 각 스펙을 시작할 때 이 파일을
다시 찾아 물을 필요 없도록 화면 ID(`dv-opt id`)를 섹션별로 적어 둔다 — 파일 안
`<div class="dv-row">섹션명</div>` 다음에 오는 `id="Nx"` 값들이며, 문구 원문은
같은 파일의 `KO`/`EN` 문자열 테이블(`welcomeTitle:` 검색 지점 부근)에 있다.

| 로드맵 번호 | 스펙 | dv-row 섹션명 | 화면 ID |
| --- | --- | --- | --- |
| 27 | 043(완료) | Launch | `1k` `1l` `1m` `1n` |
| 28 | 044 | First run / Onboarding — four feature slides while the modules download / Persona picked | `1a` (작명) · `1o` `1p` `1r` `1s` `1q`(순서대로 슬라이드 1~4·완료) · `1u` `1v` `1w`(페르소나별 동일 화면) |
| 29 | 045 | (28과 동일한 `1o`~`1q` 구간을 다운로드 전용 화면 관점에서 재확인) | `1o` `1p` `1r` `1s` `1q` |
| — | 046 | (다운로드 진행 화면 완성 — Modernist 시리즈 밖, 아래 별도 절 참고) | — |
| 28(재작업) | 047(완료) | First run — 작명 화면을 `1a`와 실제로 일치시킴(044의 어긋남 교정) | `1a` |
| 31 | 048 | Home — two layout directions (`1d` 채택, `1e` 미채택) | `1d` |
| 32 | 미착수 | Writing | `1y` |
| 33 | 미착수 | Diary detail — print-page metaphors | `1i` `1j` |
| 34 | 미착수 | (33번과 같은 `1i`/`1j` 안의 사진 슬라이더·갤러리 요소 확대 참조) | `1i` `1j` |
| 35/36 | 미착수 | (설정 섹션은 별도 dv-row 없이 홈/상세 프레임 안에 포함 — 리뷰 보드에 전용 화면 없음, 043 팔레트만 상속. 하단 탭 바 전제는 048로 사라짐) | — |
| 참고 | — | Diary detail 다음 무제 opt | `2a`(다음 상태 전이 예시) |

**044(28번) 상세 문구 원문**(마크업 KO 문자열 테이블에서 발췌, `WelcomeScreen`
재작성 시 그대로 참조):
- `1a` 작명 화면: `welcomeTitle`="깨어났어요. 처음 뵙겠습니다.", `welcomeBody`="이제부터
  제가 주인님의 하루를 사진과 다닌 자리로 읽고, 일기로 적을게요. 모든 일은 이
  휴대폰 안에서만 일어나요.", `namePrompt`="제 이름을 지어주세요.",
  `nameHint`="12자까지. 나중에 설정에서 바꿀 수 있어요."(035는 10자 상한이므로
  숫자는 035 계약을 따른다), `submit`="이 이름으로 할래요", `skip`="나중에 할래요".
- `1o`~`1s`/`1q` 온보딩 다운로드 슬라이드(4장 + 완료): `onbKicker`="준비하는 중",
  슬라이드1 `onb1Title`="쓰지 않아도 남는 하루"/`onb1Body`="따로 적을 일이
  없어요. 그날의 사진과 다닌 자리만으로 하루가 한 편 남습니다.", 슬라이드2
  `onb2Title`="나중에 다시 읽고 싶은 기록", 슬라이드3 `onb3Title`="잠들기 전에
  도착해요", 슬라이드4 `onb4Title`="휴대폰 안에서만 남아요". 진행률 문구는
  `onbProg1`~`onbProg4`처럼 "받는 중 · 310MB / 1.2GB" 형태이나 **원칙 IV 위반**
  (바이트·용량 수치 노출) — 044/045 구현 시 이 수치는 그대로 옮기지 않고 정직한
  진행률 표현으로 대체해야 한다(로드맵 45번 항목이 이미 이 점을 명시).
- `1u`/`1v`/`1w` 페르소나 픽 화면: 같은 레이아웃을 로봇(🤖, 어미 없음)·고양이(🐱,
  "냥" 어미)·강아지(🐶, "멍" 어미) 세 톤으로 보여주는 예시 — 037 기준 현재
  로스터가 하나(금동이)뿐이므로 **다중 페르소나 어미 변형 자체는 044 범위가
  아니다**(아래 26번 항목 검토 대상). 044는 레이아웃·타이포그래피만
  가져오고 어미 변형 로직은 만들지 않는다.

### 27. Modernist 디자인 시스템 적용 1차 — 스플래시·권한 요청 흐름 — ✅ 043에서 구현

- **배경 및 이전 시도 경위**: 디자인 리뷰 보드(`Alpharium Mobile Screens (standalone).html`)를 적용하려던 이전 시도(043 최초본, 2026-09-18)가 홈·상세 화면까지 한 번에 재구현하려다 기존 계약(038 타자기 연출, 017 소요 시간, 025 갤러리)과 세 차례 충돌하여 전면 롤백되었다. 그 교훈으로 범위를 스플래시와 권한 요청 흐름 하나로 좁혀 재구현했다.
- **구현 결과 (043, 2026-09-18)**:
  1. `src/ui/theme/tokens.ts`의 9개 색 역할을 Modernist 팔레트(오프화이트 `#f3f2f2`, 서피스 `#eae9e9`, 텍스트 `#201e1d`, 레드 액센트 `#ec3013`, `accentForeground` `#000000`)로 전면 교체하여 DT4 대비 테스트를 통과시켰다.
  2. `LogoScreen.tsx`를 Modernist 마크업 레이아웃(레드 스퀘어 심볼 + 브랜드 타이틀 + 하단 로딩 바)으로 재작성했다.
  3. `OnboardingScreen.tsx`의 사진·위치·알림 단계에서 목적 설명 카드와 [허용]/[건너뛰기] 버튼을 걷어내고, 시스템 다이얼로그가 자동 순차 호출되는 흐름으로 재구성했다. 배터리 예외 단계는 기존 설명/버튼 구조를 유지하되 시각 스타일만 통일했다.
  4. 상세: `specs/043-modernist-splash-permissions/`.

### 28. Modernist 디자인 시스템 적용 2차 — 첫 만남 및 캐릭터 작명 (044)

- **디자인 마크업 참조**: 위 「Modernist 디자인 리뷰 보드」 표의 화면 ID `1a`(작명)
  · `1o` `1p` `1r` `1s` `1q`(다운로드 슬라이드 1~4·완료) · `1u`~`1w`(페르소나 예시,
  044 범위 아님). 문구 원문·주의사항도 위 표 아래 발췌 참고.
- **아이디어**: 권한 요청 완료 후 나타나는 첫 만남 화면(`WelcomeScreen.tsx`)을 Modernist 브랜드 타이포그래피와 오프화이트/레드 테마로 이관합니다.
- **핵심 요구사항**:
  - `WelcomeScreen.tsx`의 3가지 상태(`checking`, `welcome`, `failed`) 레이아웃을 Modernist로 새로 작성.
  - 035의 작명 규칙(상한 10자, 빈 이름 시 기본값 폴백) 및 040의 백그라운드 병렬 다운로드 트리거 계약 준수.
  - 화면은 여전히 문자열과 콜백만 받으며 내부 식별자나 추론 객체에 닿지 않는다(원칙 III).

### 29. Modernist 디자인 시스템 적용 3차 — 필수 에셋(VLM & LLM) 다운로드 전용 화면 (045)

- **디자인 마크업 참조**: 화면 ID `1o` `1p` `1r` `1s` `1q`(위 표와 동일 구간,
  다운로드 진행률 관점). 마크업의 `onbProg1`~`onbProg4` 문구("받는 중 ·
  310MB / 1.2GB")는 원칙 IV 위반이므로 **그대로 옮기지 않는다** — 위 표 하단 발췌 참고.
- **아이디어**: 기존의 단순 스피너 및 텍스트(`WaitingForDownloadScreen.tsx`)를 대체하는 독립된 Modernist 에셋 다운로드 화면을 신설합니다.
- **핵심 요구사항**:
  - 시각 모델(VLM)과 언어 모델(LLM)의 준비 상태와 다운로드 진행률을 명확하게 안내하는 전용 화면 구성.
  - 헌법 원칙 III·IV 엄격 준수: 모델 식별자, 파라미터 수, 바이트 크기, 속도 지표를 사용자 화면에 노출하지 않고(MUST NOT), 정직한 진행률 바와 사람의 말로 표현.
  - 다운로드 완료 시 첫 일기 자동 생성 또는 일기 목록 화면으로 매끄럽게 연결.

### 30. 다운로드 진행 화면(캐러셀 + 프로그레스 바) 완성 — ✅ 046에서 구현 (2026-09-21)

Modernist 시리즈(27~29·31~36번)와 별개 — 045가 만든 화면의 **완성**이지
새 디자인 이관이 아니다. 저장소 소유자가 실기기에서 "다운로드 관련 기능이
전혀 원하는 방식대로 동작하지 않는다"고 지적해 시작됐다.

- **문제**: 045의 `DownloadProgressScreen`은 계약(`resolveSlideStage`,
  `essentialDownloadFraction`)만 갖춰져 있고 화면 자체는 미완성이었다 —
  카드가 4초 자동 타이머로만 넘어가고 스와이프 불가, 4분할 프로그레스 바가
  실제 진행률과 무관하게 정적, 실패 시 전체 화면이 별도 막다른 화면으로
  전환.
- **구현 결과**: `react-native-reanimated-carousel`(v5, `loop`+`autoplay`+
  `autoplayInterval`)을 신규 도입해 무한 순환 캐러셀을 완성 — 손 스와이프와
  4초 자동 전환이 함께 동작. 필수 peer dependency
  `react-native-gesture-handler`도 함께 도입(이 저장소 최초, `App.tsx` 루트를
  `GestureHandlerRootView`로 감쌈). `src/firstrun/consent.ts`에
  `progressSegments()`(순수 함수, 0~1 fraction → 4구간 채움 비율) 신규 추가 —
  `App.tsx`가 041/029 `essentialDownloadFraction()` 콜백 값을
  `DownloadProgressScreen`의 `downloadFraction` prop으로 그대로 전달하고,
  화면이 `progressSegments()`로 펼쳐 `react-native-reanimated`의
  `withTiming`으로 애니메이션. 프로그레스 바는 캐러셀 카드 전환과 완전히
  독립(서로 다른 state). 실패 시 전용 화면(`download-progress-failed`,
  [다시 시도] 버튼)을 제거 — 캐러셀·진행 바 레이아웃을 유지한 채 진행 바
  하단 문구만 "받다가 멈췄어요"로 교체, `App.tsx`가 10초 간격으로 스스로
  재시도(`onRetry` prop 제거, 재시도는 조립 계층 책임).
- **구현 중 실측 정정**: 라이브러리 문서 예제는 `autoPlay`(camelCase)로
  표기하나 실제 설치된 v5.1.1 타입 선언은 `autoplay`(소문자) —
  `tsc`가 즉시 잡아냈다(007·014의 "타입 축소는 tsc가 짚어준다" 관례
  재확인).
- **테스트 함정**: `react-native-reanimated-carousel`이 내부적으로
  `react-native-worklets`를 다시 require해 033이 만든 `react-native-
  reanimated` jest 목을 우회하고 같은 `loadUnpackers` 오류로 죽는다 —
  `jest/setup-ui.ts`에 이 라이브러리 전용 목(첫 카드만 1회 렌더하는 최소
  구현)을 추가해 해소했다. 무한 순환·스와이프 등 실제 캐러셀 동작은 이
  목으로 검증 불가(033 PF7과 같은 대가) — 실기기 검증으로 보완.
- **헌법 정합성**: 헌법 2.0.0 원칙 IV의 명시적 허용("진행 인디케이터·
  다운로드 상태 표시는 금지 대상이 아니다")과 정확히 부합 — 새 헌법 개정
  불필요. Constitution Check 사전·사후 PASS.
- `/speckit-specify`~`/speckit-implement` 전 체인 수행, `/speckit-analyze`
  1회 반복으로 발견한 이슈(HIGH 1·MEDIUM 2) 전부 해소 후 구현. 기기 없는
  테스트 156 스위트 2781개 통과, lint(eslint·tsc·헌법 검사·prettier) 클린.
  상세: `specs/046-download-progress-carousel/`.
- 실기기 검증(dev debug, 신규 네이티브 링크 모듈 도입이므로 AGENTS.md 기준
  필수)은 이 세션에서 아직 수행하지 않았다 — quickstart.md의 6단계 절차로
  다음 세션에서 수행 예정.

### 31. Modernist 디자인 시스템 적용 — 일기 목록 및 쓰기 진입

- **디자인 마크업 참조**: 화면 ID `1d` `1e`("Home — two layout directions", 두
  레이아웃 대안 중 선택 필요). 원본 파일 경로는 위 「디자인 리뷰 보드」 절 참고.
- **아이디어**: 일기 탭의 기본 뷰인 일기 목록(`DiaryListScreen.tsx`)과 날짜 선택 컴포넌트(`DayPicker.tsx`)를 Modernist 디자인 시스템으로 재구현합니다.
- **핵심 요구사항**:
  - 상단 날짜 선택 바(`DayPicker`): 오프화이트 배경과 라인/액센트 기반 날짜 선택 UI.
  - 쓰기 버튼 영역: 정오 게이트 안내(012), 작성자 안내, "일기 쓰기" 버튼의 미니멀 스타일화.
  - 일기 카드 목록: 날짜, 작성자 이름, 사진 힌트가 들어간 카드 레이아웃 및 일기 없을 때의 빈 상태 화면.
  - 006 S1 계약 유지: 읽기와 쓰기가 결합되지 않으며 `onWrite`는 목록을 보지 않는다.
- **진행 기록 (048, 2026-09-24)**: `1d`(Day-first)를 채택하고 `1e`는 채택하지 않았다.
  `1d`는 하단에 쓰기 바가 고정되는 구조라 맨 위 탭 줄과 공존할 수 없어 **화면 이동 구조를
  함께 바꿨다** — 탭 줄을 없애고 홈을 뿌리로, 설정·개발자는 하단 바 `⋯` 메뉴로 들어가는
  하위 화면으로. 7칸 스트립·신호 줄(사진·다닌 자리·쓸 수 있는 때)·아직 쓸 수 없는 오늘(정오
  전 · 새벽)의 안내와 전환이 들어왔다. 설계: `docs/superpowers/specs/2026-09-24-diary-home-modernist-design.md`,
  스펙: `specs/048-diary-home-modernist/`.

### 32. Modernist 디자인 시스템 적용 — 일기 작성 진행 연출 및 보조 화면

- **디자인 마크업 참조**: 화면 ID `1y`("Writing"). 원본 파일 경로는 위 「디자인
  리뷰 보드」 절 참고. 마크업의 `writingBy`(경과 시간 미표기, "OO이 쓰고 있어요.
  진행률은 세지 않아요") 문구가 015/016 계약과 부합.
- **아이디어**: 일기 쓰기 트리거 후 일기가 생성되는 동안의 진행 화면 및 관련 예외 모달들을 Modernist로 정비합니다.
- **핵심 요구사항**:
  - `DiaryHomeScreen.tsx`의 `writing` 상태: 미니멀 인디케이터 + 039 독백 타자기 연출(`TypewriterText`) + [그만두기] 버튼. 진행률 숫자·경과 시간은 일체 미노출(015/016 유지).
  - 덮어쓰기 확인 다이얼로그(`OverwriteConfirmScreen.tsx`): 오늘/선택한 날의 기존 일기 덮어쓰기 경고 및 확인/취소 액션.
  - 에러 및 안내 화면(`BuildErrorScreen.tsx`, 손상된 일기 안내).
  - 038 타자기 연출 및 독백 갱신 계약 엄격 보호.

### 33. Modernist 디자인 시스템 적용 — 일기 상세 본문 및 관측 신호

- **디자인 마크업 참조**: 화면 ID `1i` `1j`("Diary detail — print-page
  metaphors"). 원본 파일 경로는 위 「디자인 리뷰 보드」 절 참고. `sawTitle`·`saw1`
  ~`saw3`(소요 시간·장소 요약 문구)가 017/헌법 1.2.0 사후 1회성 노출 규범과 대응.
- **아이디어**: 일기 전문 화면(`DiaryDetailScreen.tsx`)의 텍스트 본문 레이아웃과 관측 신호 메타데이터 영역을 Modernist 타이포그래피로 구현합니다.
- **핵심 요구사항**:
  - 상단 날짜, 작성자 이름, 일기 본문 텍스트 타이포그래피 (신문/문학적 감성의 레이아웃).
  - 038 계약 유지: 생성 직후 첫 노출(`reveal === true`) 시에만 타자기 연출 적용, 목록에서 열 때는 즉시 노출.
  - 017 계약 유지: "이 일기가 본 것" 메타데이터 섹션(장소, 걸음 수, 배터리 등) 및 헌법 1.2.0 사후 1회성 소요 시간 노출 규범 준수.
  - 사진 영역(슬라이더/갤러리)은 기존 구조를 유지하거나 자리만 마련하고 34번에서 다룬다.

### 34. Modernist 디자인 시스템 적용 — 사진 슬라이더 및 풀스크린 갤러리 모달

- **디자인 마크업 참조**: 화면 ID `1i` `1j`(33번과 공유하는 프레임 안의 사진
  슬라이더/캡션 요소, 리뷰 보드에 갤러리 모달 전용 화면은 별도로 없음 — 025
  기존 계약과 043 팔레트로 재구성).
- **아이디어**: 일기 상세 화면 내의 사진 탐색 컴포넌트를 Modernist 미학에 맞춰 정교하게 재구현하되, 025 갤러리 계약을 단독으로 완벽히 검증합니다.
- **핵심 요구사항**:
  - 가로 스크롤 사진 슬라이더: 025 코어 RN `ScrollView` 기반 가로 페이징 썸네일 및 `1 / 8` 순번 텍스트.
  - 풀스크린 갤러리 모달: 사진 탭 시 진입하는 풀스크린 미디어 뷰어(`Modal`, 순수 블랙 배경, 가로 스와이프, 뒤로가기/닫기).
  - 새 네이티브 의존성 추가 없이 코어 RN 컴포넌트로만 구현(025 계약 엄수).

### 35. Modernist 디자인 시스템 적용 — 설정 화면 기본 섹션 (전역 하단 탭 바 전제는 048로 사라짐)

- **디자인 마크업 참조**: 리뷰 보드에 이 화면들만을 위한 전용 `dv-opt`가 없다 —
  홈/상세 프레임(`1d` `1e` `1i` `1j`) 안에 곁들여진 탭 바·설정 요소를 참고하고,
  043이 이미 이식한 팔레트(`src/ui/theme/tokens.ts`)를 그대로 상속한다.
- **아이디어**: 설정 화면의 상단 기본 섹션들을 Modernist 룩앤필로 이관합니다.
- **전제 정정 (048)**: 이 항목이 전제한 「전역 하단 탭 바」는 없다 — 048이 탭 줄을 없애고
  설정·개발자를 홈 하단 바의 `⋯` 메뉴로 들어가는 하위 화면(`SubScreenFrame`, 「← 일기」)으로
  바꿨다. 이 항목은 그 하위 화면 **안**의 내용만 다룬다.
- **핵심 요구사항**:
  - 일기 작성자 선택기(`AuthorPicker.tsx`): 작성자 목록, 선택 상태, 인라인 작명 에디터.
  - 일반 설정 토글: 장소명 설정(`GeocodingSettingToggle.tsx`), 자동 일기 설정(`AutoDiarySettingsScreen.tsx`).

### 36. Modernist 디자인 시스템 적용 — 설정 탭 모델 관리 및 권한 관리

- **디자인 마크업 참조**: 34번과 동일 — 리뷰 보드에 전용 화면 없음, 043 팔레트를
  상속해 기존 `CharacterListScreen.tsx`·`PermissionsSection.tsx` 구조 위에 스타일만 입힌다.
- **아이디어**: 설정 탭의 하단에 위치한 모델 다운로드/저장소 관리 및 시스템 권한 관리 섹션을 Modernist로 완성합니다.
- **핵심 요구사항**:
  - 모델 다운로드 관리(`CharacterListScreen.tsx`, `ModelSection.tsx`): 캐릭터별/비전 모델별 용량, 다운로드/이어받기 진행률, 모델 삭제 액션.
  - 권한 관리 섹션(`PermissionsSection.tsx`): 필수 권한 상태 목록 및 온보딩 재실행 버튼.
  - 008 다운로드 충돌 방지 및 026 세그먼트 이어받기 계약 유지.
