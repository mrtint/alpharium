...
- [x] T019 [P] [US3] `__tests__/models/segmented-transfer.test.ts` 작성 — contracts/
...
- [x] T020 [US3] `src/models/segmented/transfer.ts` 구현 — `runSegmented(deps, key, url, opts)`
...
- [x] T021 [US3] `src/models/expo-port.ts` 수정 — `expoDownloadPort().start()`의
...
=> void`로 고정 — 세그먼트 코어가 바이트를 모른다. **`__DEV__` 강제 폴백 스위치 추가**
      ...
      취급, segmented-transfer.md 「**DEV** 강제 폴백」, SC-004·Q3·Q4 공용). `{ paused, resume }`
      → `{ kind: "paused", state: resume }`.
- [x] T022 [US3] `src/models/expo-port.ts`에 `expoRangeFetchPort()` 구현 —
...
- [x] T023 [US3] `src/models/acquisition.ts` — `prepare`의 `download.start`/`download.resume`
...
- [x] T024 [P] [US2] `__tests__/models/segmented-transfer.test.ts`에 병렬 수신 케이스
...
- [x] T025 [P] [US2] `__tests__/models/segmented-transfer.test.ts`에 원칙 III·IV 검사 —
...
- [x] T026 [US2] `src/models/segmented/transfer.ts` — `runSegmented`의 `Promise.all`
      병렬 실행이 T024를 만족하도록 확정 (T020에서 이미 병렬이면 검증만; 순차였으면 병렬로).
      구간 실패 시 `AbortController`로 나머지 즉시 취소. T024 GREEN.
- [x] T027 [US2] `src/models/expo-port.ts` `expoRangeFetchPort().fetchRange` — 청크
...
- [x] T028 [US2] `src/models/segmented/plan.ts` 주석 — `SEGMENT_COUNT`·`MIN_SEGMENT_BYTES`
...
- [x] T029 [P] [US4] `__tests__/models/segmented-plan.test.ts`에 재개 계산 보강 —
      `remainingSegments`가 부분 구간의 `start`를 `receivedBytes[i]`만큼 밀고 완료 구간은
      제외 (C4·C5 심화: 여러 구간이 서로 다른 진행도일 때).
- [x] T030 [P] [US4] `__tests__/models/concurrent-acquisition.test.ts`에 세그먼트 재개
...
- [x] T031 [US4] `src/models/readiness.ts` 수정 — `ReadinessInput`에
...
- [x] T032 [US4] `App.tsx`의 `ModelSection.read()` 수정 — 준비 상태 재료 수집 시
      `segmentedFor(state, asset.key)`를 읽어 `readinessOf`에 `segmentedResume` 전달
      (기존 `verdictFor`·`pausedFor` 옆).
- [x] T033 [US4] `src/models/expo-port.ts` `expoDownloadPort().resume()` 수정 —
...
- [x] T034 [US4] `src/models/acquisition.ts` `prepare`의 7-a (pause 처리) 확정 —
...
- [x] T035 [P] `npm run lint` 클린 확인 — eslint 0 error, `tsc` 통과 (특히
...
- [x] T039 [P] `specs/026-parallel-model-download/findings.md` 작성 — quickstart.md
      「findings.md 채울 항목」 §1~§7. 미확인 잔여(prod 게이트, release 재확인 판정 —
      새 네이티브 모듈 없으므로 debug 1회로 충분, 012 기준 명시).
- [x] T040 [P] `docs/roadmap/README.md` 갱신 — 10번 항목에 `(026 — 동시 다운로드 +
세그먼트 병렬 + 폴백 + 세그먼트 이어받기)` 표기, 「과제별 상세」 10번에 구현 결과 문단
      (023·024·025 항목과 같은 형식).
- [x] T041 `specs/003-character-model-files/contracts/acquisition.md`의 「한 번에 하나
