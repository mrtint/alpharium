...
- [~] T029 [US1] SM-S928N(One UI 8.5)에 **부모 테마 교체 반영 debug APK**(`AppTheme` parent = `Theme.AppCompat.Light.NoActionBar`, 커밋 `837b1ef`) 설치 후 `cmd uimode night yes`. **✅ 관측(2026-09-03)**: 온보딩 1단계·에셋 다운로드 단계·"1/4" 재게이트 전부 배경 `rgb(250,250,250)` + 텍스트 검정 + 대비 또렷. `#303030`(1·2차 실패값) 재현 0건. `cmd uimode night no`에서 `rgb(250,250,250)` 동일(회귀 없음). **미확인**: 목록·상세·설정·개발자 탭 — 에셋(~2GB 모델) 미준비로 온보딩 게이트를 못 넘음(029 model-not-ready). 테마는 앱 전역(윈도우 레벨 `AppTheme`)이라 동일할 것이나 실측은 다음 세션(모델 배치 후). `cmd uimode night auto` 복원 완료. per FR-001·SC-001
- [x] T030 [US2] SM-S928N에서 온보딩 흐름 확인(2026-09-03). **✅**: `uiautomator dump`로 4단계가 `onboarding-step-photos` → `onboarding-step-location` → `onboarding-step-notifications` → `onboarding-step-battery-exception` 순서로 정확히 나오고 **`onboarding-step-photo-location` 부재**. 4개 전부 [건너뛰기]로 통과 → `onboarding-step-assets`(에셋 다운로드) 도달 — 어느 권한 단계에서도 갇힘 없음. **미확인**: 설정 "권한" 섹션 행 4개(에셋 게이트로 설정 탭 미도달, 다음 세션). per FR-006·007·009·SC-002·003
...
- [ ] T037 **다음 실기기 세션**: 검증용 모델 4개(`a1`·`a2`·`v1`·`v2` 또는 온보딩 에셋 다운로드) 배치 → `cmd uimode night yes` → **목록·상세·설정·개발자 탭 4화면**이 밝은 배경 + 대비인지 확인(`#303030` 아님), 설정 "권한" 섹션 행 4개(`permission-row-photo-location` 부재), T023(신호 수집 회귀), T024(Maestro `node scripts/run-device-tests.mjs`), T025(release 재확인 — prebuild + `NODE_ENV=production ./gradlew assembleRelease` + `apksigner verify` + 새 설치 + 다크 모드 확인, `expo-system-ui` minify 생존). per T029·T030 잔여 + tasks T023·T024·T025
