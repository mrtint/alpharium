...
- [ ] **Modernist 디자인 시스템 적용 4차 — 일기 목록 및 쓰기 진입** (046 예정 — `DiaryListScreen` + `DayPicker`. 상단 날짜 선택 바, 일기 쓰기 버튼 영역, 일기 카드 목록, 빈 상태 뷰. 아래 30번 상세)
- [ ] **Modernist 디자인 시스템 적용 5차 — 일기 작성 진행 연출 및 보조 화면** (047 예정 — `DiaryHomeScreen`의 `writing` 독백 타자기 연출 및 회전 인디케이터, `OverwriteConfirmScreen`, `BuildErrorScreen`. 038 타자기 및 독백 갱신 계약 엄격 보호. 아래 31번 상세)
- [ ] **Modernist 디자인 시스템 적용 6차 — 일기 상세 본문 및 관측 신호** (048 예정 — `DiaryDetailScreen` 본문 Modernist 타이포그래피, 038 생성 직후 타자기 첫 노출 연출(`reveal`), 017 "이 일기가 본 것" 메타데이터 및 소요 시간 표기. 아래 32번 상세)
- [ ] **Modernist 디자인 시스템 적용 7차 — 사진 슬라이더 및 풀스크린 갤러리 모달** (049 예정 — `DiaryDetailScreen` 내 025 갤러리 계약 단독 격리. 코어 RN 가로 슬라이더 + 풀스크린 모달 갤러리 뷰어. 아래 33번 상세)
- [ ] **Modernist 디자인 시스템 적용 8차 — 전역 하단 탭 바 & 설정 기본 섹션** (050 예정 — Modernist 전역 하단 탭 바, `AuthorPicker`, 일반 설정 토글(`Geocoding`, `AutoDiary`). 아래 34번 상세)
- [ ] **Modernist 디자인 시스템 적용 9차 — 설정 탭 모델 관리 및 권한 관리** (051 예정 — `CharacterListScreen` / `ModelSection` 모델 다운로드/삭제 관리, `PermissionsSection` 권한 상태 카드 및 온보딩 재실행. 아래 35번 상세)
- [~] **프롬프트로 화자를 바꾼다 — 강아지·고양이 페르소나** (`my-ollama` 후속 연구 `results/pet-speaker/`(커밋 `a39fae6`, kanana 120런) 이식. 모델은 kanana 그대로 두고 **프롬프트만으로 화자를 바꾼다** — 036이 이식한 E2SN의 뼈대·인식 구조·금지 조항을 유지하고 말투 층만 갈아 끼운다. **헌법 v1.8.0 선행 개정 완료**(2026-09-17): 페르소나가 프롬프트에서 말투·태도를 지시해도 되고(MAY), 한 모델 위에 캐릭터가 여럿 설 수 있다(MAY). 용어 「씨앗」→「모델의 성질」. **구현은 별도 스펙(Modernist 완료 후 052 예정)** — 진입 기준은 캐릭터마다 실기기 관측이라 my-ollama 120런은 후보를 좁힌 것이지 근거가 아니다. 아래 26번 상세)
...
- **구현은 별도 스펙(Modernist 완료 후 052 예정), 아직 열지 않았다.** 헌법은 길을 열 뿐이고 **실기기
  관측이 들여보낸다.** 스펙이 다룰 것:
...
| 30 | 046 | Home — two layout directions | `1d` `1e` |
| 31 | 047 | Writing | `1y` |
| 32 | 048 | Diary detail — print-page metaphors | `1i` `1j` |
| 33 | 049 | (048과 같은 `1i`/`1j` 안의 사진 슬라이더·갤러리 요소 확대 참조) | `1i` `1j` |
| 34/35 | 050/051 | (하단 탭 바·설정 섹션은 별도 dv-row 없이 홈/상세 프레임 안에 포함 — 리뷰 보드에 전용 화면 없음, 043 팔레트만 상속) | — |
...
- `1u`/`1v`/`1w` 페르소나 픽 화면: 같은 레이아웃을 로봇(🤖, 어미 없음)·고양이(🐱,
  ...
  아니다**(052 검토 대상, 아래 26번 항목 참고). 044는 레이아웃·타이포그래피만
...
### 30. Modernist 디자인 시스템 적용 4차 — 일기 목록 및 쓰기 진입 (046)
...
### 31. Modernist 디자인 시스템 적용 5차 — 일기 작성 진행 연출 및 보조 화면 (047)
...
### 32. Modernist 디자인 시스템 적용 6차 — 일기 상세 본문 및 관측 신호 (048)
...
- **핵심 요구사항**:
  ...
  - 사진 영역(슬라이더/갤러리)은 기존 구조를 유지하거나 자리만 마련하고 049에서 다룬다.

### 33. Modernist 디자인 시스템 적용 7차 — 사진 슬라이더 및 풀스크린 갤러리 모달 (049)
...
### 34. Modernist 디자인 시스템 적용 8차 — 전역 하단 탭 바 & 설정 기본 섹션 (050)
...
### 35. Modernist 디자인 시스템 적용 9차 — 설정 탭 모델 관리 및 권한 관리 (051)
