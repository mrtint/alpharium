...
## 버전 상태 — 선언과 설치본이 어긋나 있다

2026-08-12 실측:

| | `package.json` 선언 | `node_modules` 설치본 |
| --- | --- | --- |
| `expo` | `~54.0.36` | **57.0.12** |
| `react-native` | `0.81.5` | **0.86.2** |
| `llama.rn` | 선언 없음 | **0.12.9** |

설치본은 온디바이스 추론이 실증된 조합에 이미 맞아 있고, 선언 쪽만 뒤처져 있다.
`llama.rn`은 선언 없이 설치되어 있다. **어느 쪽으로 맞출지는 스펙 단계에서 정한다.**
그 전에 임의로 한쪽을 지우거나 덮어쓰지 않는다.
