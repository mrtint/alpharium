# Implementation Plan: 쓰는 중 독백 확장 — 콜드/핫 스타트·사진 보기·문구 폭
...
### 설계에서 새로 드러난 위험 넷 — Phase 0 조사 및 `/speckit-analyze`에서 확인·해소
...
3. **사진 보기 갈래(많음/보통)를 015의 `PhotoAdvanceSignal`(인자 없음)
   ...
   변경하지 않는다.
4. **모델 로드 구간에서 "그만두기"가 실제로 무엇을 하는가**(2026-08-23
   `/speckit-analyze` C1로 발견 — FR-013이 태스크로 전혀 커버되지 않은
   상태였다) — research.md §7이 `llama.rn`의 타입 선언을 실측해 로드를
   중단할 API 자체가 없음을 확인했다. `captionAll()`이 이미 쓰는 "완료
   후 취소 재확인" 패턴(`caption.ts:156`)을 로드에도 적용해, 로드가
   끝나는 즉시 취소 상태를 확인하고 `{ kind: "interrupted" }`로 버리는
   것으로 설계했다(`contracts/load-signal.md`, tasks.md T013a·T013b).
   "즉시 중단"이 아니라는 사실을 spec.md Edge Cases·FR-013에 정직하게
   반영했다(원칙 II·V).

네 위험 모두 "실제로 아는 시점에만 말한다"(원칙 II·V)와 "재지 않고 이미
