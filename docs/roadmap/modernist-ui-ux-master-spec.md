...
스플래시 및 온보딩부터 일기 목록, 작성 진행 연출, 상세 본문, 갤러리, 설정 탭에 이르는 전체 사용자 여정을 다루며, `docs/roadmap/README.md`의 세부 로드맵 과제(27~29번·31~36번)와 1:1로 직결되도록 구성되었습니다.
...
| **28** | `044` | [§4.2 첫 만남 및 캐릭터 작명](#42-첫-만남-및-캐릭터-작명-로드맵-28번--044) | `WelcomeScreen` (3가지 상태: `checking`, `welcome`, `failed`) | `1a` | 완료 (PR #73, 047 PR #81에서 `1a`와 일치시킴) |
| **29** | `045` | [§4.3 필수 에셋 다운로드 전용 화면](#43-필수-자산-다운로드-동의-및-진행-화면-로드맵-29번--045) | `DownloadConsentDialog`, `DownloadProgressScreen` (슬라이드 4종 + 진행률 바) | `1o`~`1s`, `1q` | 완료 (PR #74, 046 PR #80에서 캐러셀·진행 바 완성) |
| **31** | — | [§4.4 일기 목록 및 쓰기 진입](#44-일기-목록-및-쓰기-진입-로드맵-31번) | `DiaryListScreen`, `DayPicker`, 일기 카드, 빈 상태 뷰 | `1d`, `1e` | 미착수 |
| **32** | — | [§4.5 일기 작성 연출 및 보조 화면](#45-일기-작성-진행-독백-연출-및-보조-화면-로드맵-32번) | `DiaryHomeScreen`(writing 독백), `OverwriteConfirm`, `BuildError` | `1y` | 미착수 |
| **33** | — | [§4.6 일기 상세 본문 및 관측 신호](#46-일기-상세-본문-및-관측-신호-로드맵-33번) | `DiaryDetailScreen` (타이포그래피, 타자기 reveal, "본 것" 메타) | `1i`, `1j` | 미착수 |
| **34** | — | [§4.7 사진 슬라이더 및 풀스크린 갤러리](#47-사진-슬라이더-및-풀스크린-갤러리-모달-로드맵-34번) | `DiaryDetailScreen` 내 가로 페이징 슬라이더, 코어 RN 모달 갤러리 | `1i`, `1j` 발췌 | 미착수 |
| **35** | — | [§4.8 전역 하단 탭 바 & 설정 기본 섹션](#48-전역-하단-탭-바--설정-기본-섹션-로드맵-35번) | 전역 탭 바, `AuthorPicker`(인라인 작명), `Geocoding`, `AutoDiary` | 홈/상세 프레임 공통 | 미착수 |
| **36** | — | [§4.9 설정 탭 모델 관리 및 권한 관리](#49-설정-탭-모델-관리-및-권한-관리-로드맵-36번) | `CharacterListScreen`, `ModelSection`, `PermissionsSection` | 043 팔레트 상속 | 미착수 |
...
1. **착수 전 `/using-superpowers` 브레인스토밍 필수 (MUST)**:
   - 각 로드맵 과제(31~36번) 착수 시, 코드를 작성하기 전에 반드시 `/using-superpowers` (`brainstorming` 스킬)을 실행합니다.
...
- **상태**: 구현 및 실기기 검증 완료 (PR #73), 047(PR #81)에서 `1a`와 일치시킴
...
- **상태**: 구현 및 실기기 검증 완료 (PR #74), 046(PR #80)에서 캐러셀·진행 바 완성
...
### 4.5 일기 작성 진행 독백 연출 및 보조 화면 (로드맵 32번)
- **상태**: 미착수
...
### 4.7 사진 슬라이더 및 풀스크린 갤러리 모달 (로드맵 34번)
- **상태**: 미착수
...
### 4.8 전역 하단 탭 바 & 설정 기본 섹션 (로드맵 35번)
- **상태**: 미착수
...
### 4.9 설정 탭 모델 관리 및 권한 관리 (로드맵 36번)
- **상태**: 미착수
