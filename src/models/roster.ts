 ...
 *  3. `allAssets()`를 두지 않는다(FR-010). 있으면 "로스터를 통째로 받자"가 한 줄로
 *     가능해지고, 그것이 헌법 로스터가 금지한 구조다. **로스터가 하나여도 두지
 *     않는다**(037 FR-014) — 둘째가 들어올 때 이 방어선을 새로 만들어야 한다
 ...
 * 것을 그대로 옮겼다. **2026-08-14에 HTTP 200을 확인했다**(그때는 다섯이었다).
 *
 * `expectedBytes`는 그 응답의 `Content-Length`이며, **기기에 남아 있던 옛 GGUF 파일의
 * 바이트 수와 정확히 일치했다.** 두 경로가 같은 값을 주었으므로 이 숫자는 관측된
 * 사실이다.
 *
 ...
 *
 * 로스터에 남은 캐릭터는 지문이 채워져 있다 — **채록이 아니라 검증**을 한다.
 *
 * **양자화는 Q4_K_M이고 우리가 만든 것이 없다.** kanana는 커뮤니티 배포본이다(공식이
 * GGUF를 내지 않음). 직접 양자화한 것은 옆 저장소의 Ollama 데스크톱 쪽이었고 모바일
 * GGUF는 기성품이다.
 * ─────────────────────────────────────────────────────────────────────────────
 */
const ASSETS: Readonly<Record<Character, ModelAsset>> = {
  ...
  // 037(헌법 1.6.0): a2(exaone)·a3(hyperclovax)·a4(qwen3)·a5(gemma3)가
  // 로스터를 나갔다. 자동 생성에 부적합이 실측으로 확정됐다 —
  // narrative(헤드리스 완주 불가, 024 T034), imaginative(되뇜·주인화·
  // 지어내기 3/3, 037 실기기), chinese(추론 블록이 예산 소진, 028),
  // english(지시 이행 불안정, 028). 헌법 개정 기록에 출처가 있다.
  //
  // **이미 내려받은 파일을 앱이 지우지 않는다**(037 FR-011). 자산 목록에서
  // 사라지므로 다운로드 관리 화면에도 나타나지 않는다 — 사용자가 앱으로는
  // 지울 수 없는 알려진 빈자리다(008의 "받다 만 모델" 선례).
};
