...
const PERSONAS: Readonly<Record<Character, Persona>> = {
  ...
  // 로스터가 이 성질의 고지를 MUST로 요구하며, 1.1.1에서 「소개 문구 자체가
  // 강점의 언어로 그 사실을 담으면 별도 고지가 필요 없다」고 명시했다.
  // 「상상력이 풍부해요」가 그 문구다 — 헌법 로스터가 이 표현을 「상상을
  // 섞는다」와 같은 사실을 전하는 예로 직접 든다. CharacterPicker는 이제
  // 이 tagline 하나만 그리고 별도 고지 줄을 두지 않는다.
