...
- **FR-015**: VLM 쪽(`src/vision/`)은 건드리지 않는다. 캡션 언어는 영어 그대로다
  (결정 3, §4.2).
- **FR-016**: 캡션 한계 줄 중 **`VISION_UNREAD`·`VISION_NONE_READ`의 현행 문안은
  한국어 캐릭터도 그대로 쓴다**(`visionLimitLines()` 재사용) — "보려 한 사진 중
  일부는 내용을 보지 못했다.", "사진은 있었으나 내용을 하나도 보지 못했다.". 이
  둘은 §5.6 실험에서 문안이 바뀌지 않았다. **`VISION_PARTIAL`만** 문장형 흐름의
  `S_VISION_PARTIAL`("사진이 더 있었지만 그중 몇 장만 보았다.")로 교체한다. 캡션이
  하나도 없을 때 "사진이 없었다"와 구분되는 `VISION_NONE_READ`는 유지한다(SC-006
  계열). `visionLimitLines()`를 언어로 분기하되 `S_VISION_PARTIAL` 한 줄만
  갈라진다.
...
- **SC-007**: 생성 시간(writingMs)이 **현행(`base`) 대비 늘어나지 않는다** — E2SN
  출력 중앙값 416자 < base 464자(BA는 276자로 더 짧지만 E2SN은 짐작을 담아 그보다
  길다). 리포트 추정 디코드 −20초(결정 7)는 base 기준이다. 실측 1회로 "느려짐
  회귀가 없다"만 확인한다. BA 대비 빠름을 요구하지 않는다.
