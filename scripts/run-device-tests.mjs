...
const FLOWS = [
  ...
  // 011 → 042 — 사진의 내용을 본다. **042에서 전제가 뒤집혔다**: 「고르는 자리가
  // 있는가」가 아니라 **「고를 자리가 없는가」**를 본다(헌법 v1.7.0 — 사용자가 끄는
  // 경로를 두지 않는다 MUST NOT). 모델 준비 줄(`vision-row`)은 그대로 유지한다.
  // ⚠️ **이 기능의 핵심 검증은 여기 없다** — 사진 있는 하루가 실제로 캡션을 도는지는
  // `adb logcat`의 `has_media=1`로 봐야 하고, 백그라운드·진단 경로는 잡을 강제
  // 실행해야 한다. quickstart D2·D3·D5·D6을 손으로 확인한다.
  // **건너뛴 것은 통과가 아니다**(원칙 V).
  ...
  // 아니다**(원칙 V).
  //
  // ★ 037 — **이 흐름은 로스터가 하나인 동안 PASS할 수 없다.** 동시 내려받기를
  // 보려면 아직 받지 않은 캐릭터가 **둘** 필요한데(`english`·`chinese`를 썼다)
  // 헌법 1.6.0이 넷을 로스터에서 거둬들였다. 흐름을 지우지 않고 남긴다 —
  // 진입 기준을 통과한 캐릭터가 들어오면 그때 다시 돈다(037 FR-014).
  // 그때까지 026의 동시 다운로드는 quickstart Q0~Q6를 손으로 확인한다.
  ...
  // ⚠️ **정오 전 오늘 선택·12:00 전환·신호 줄 숫자 대조·prod 메뉴는 여기 없다** — 기기
