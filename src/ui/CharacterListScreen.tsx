 ...
 *
 * **추천하거나 미리 고르지 않는다**(FR-005b). 로스터의 캐릭터가 같은 자격으로 보인다 — 추천에는
 * 근거가 필요하고 그 근거가 없다.
 * ─────────────────────────────────────────────────────────────────────────────
 */
 ...
 *
 * **`ListRow`의 기본값은 14다.** 캐릭터 행 + 사진 모델 행이 2px씩 커지면 누적
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/*
        로스터의 자리가 **처음부터 전부** 보인다(FR-005a). 아무것도 준비되지 않은 첫
        화면에서도 마찬가지이며, 고를 대상이 보이지 않으면 고를 수 없다. 037로
        로스터가 하나가 됐으나 `CHARACTERS`를 도는 구조는 그대로다 — 캐릭터가 늘면
        줄도 는다.
      */}
      ...
      {/*
        **사진을 보는 데 필요한 것**(011 FR-026·031a).
        ...
        캐릭터 아래에 따로 온다 — **캐릭터가 아니기 때문이다**(FR-025). 한 번
        준비하면 어느 캐릭터로도 사진을 본다(SC-008).
