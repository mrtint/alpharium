 ...
 * 말없이 집었다.** 여럿을 준비해 두어도 사용자는 어느 캐릭터가 자기 일기를 썼는지
 * 모르고 바꿀 방법도 없었다 — **로스터가 여럿인 이유가 화면에서 사라져 있었다.**
 *
 * **037로 로스터가 하나가 된 뒤에도 이 규칙을 지운다는 뜻이 아니다**(FR-014) —
 * 하나뿐이어도 고른 적 없으면 자동으로 고르지 않는다. 지우면 둘째가 들어올 때
 * 006의 결함이 그대로 돌아온다.
 *
