...
- [x] SC-004: S901N 라이트 모드·온보딩 회귀 0건 — **확인 완료(2026-09-11, SM-S901N One UI 8.0)**: `night no`에서 목록·상세·설정·개발자 탭 아이보리 + 검정 텍스트, 031 전과 동일. `night yes`에서 4화면 픽셀 동일(회귀 0건).
...
- [x] SC-006: 사진 좌표→지명 동작 수정 전후 동일 — **확인 완료(2026-09-11, SM-S901N)**: `seed:day rich 2026-09-10`(좌표 3장) + 사진 auto로 생성 2회 — 위치 권한 허용 시 `placeName={"kind":"known","value":"중구"}` + 본문 "중구", `pm revoke` 후 `placeName=null` + 본문 지명 없음. `signalsUsed.places`는 두 경우 다 trace 있음(좌표는 사진 EXIF). 021 T030 원래 관측과 동일.
